...
async fn test_order_lifecycle() {
    ...
    // 2. Create a limit order (V2) at an extreme price so it won't fill (1 cent YES)
    ...
    let result = async {
        ...
        // 4. Amend the order (V2, change price to 2 cents)
    ...
    // 6. Cancel the order (V2, cleanup) - use the potentially amended order_id
...
async fn test_batch_order_lifecycle() {
    ...
    // 2. Batch create (V2) 2 limit orders at extreme prices
    let make_order = |price: &str| CreateOrderV2Request {
        ticker: market_ticker.clone(),
        side: BookSide::Bid,
        count: "1".to_string(),
    ...
    let batch_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .batch_create_orders_v2(BatchCreateOrdersV2Request {
                orders: vec![make_order("0.01"), make_order("0.02")],
            })
            .await
    })
    .await
    ...
    // 3. Batch cancel (V2) all created orders (cleanup)
    let cancel_resp = tokio::time::timeout(common::TEST_TIMEOUT, async {
        client
