...
fn v2_order(ticker: String, price: &str) -> CreateOrderV2Request {
...
async fn test_order_lifecycle() {
    ...
    let create_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .create_order_v2(v2_order(market_ticker.clone(), "0.01"))
            .await
    })
    .await
    .expect("timeout")
    ...
    let result = async {
        ...
        let amended_order_id = amend_resp.order_id.clone();

        // 5. Get queue position for the order
        let _queue_resp = tokio::time::timeout(common::TEST_TIMEOUT, async {
            client.get_order_queue_position(&amended_order_id).await
        })
        .await
        .expect("timeout")
    ...
    let batch_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .batch_create_orders_v2(BatchCreateOrdersV2Request {
                orders: vec![
                    v2_order(market_ticker.clone(), "0.01"),
                    v2_order(market_ticker.clone(), "0.02"),
                ],
            })
            .await
    })
