...
use kalshi_fast::{
    ...
    BatchCreateOrdersV2Request, BookSide, CancelOrderV2Params, CreateOrderGroupRequest,
    CreateOrderV2Request, GetMarketsParams, GetOrdersParams, MarketStatusQuery,
    SelfTradePreventionType, SubaccountQueryParams, TimeInForce, UpdateOrderGroupLimitRequest,
};
...
async fn test_order_lifecycle() {
    ...
    // 2. Create a limit order at an extreme price so it won't fill (1 cent bid), via V2
    ...
    let result = async {
        ...
        // 4. Amend the order (change price to 2 cents), via V2
    ...
    // 6. Cancel the order (cleanup), via V2 - use the potentially amended order_id
    ...
    // 7. Verify order is cancelled via get_orders, filtered to exchange shard 0
...
async fn test_batch_order_lifecycle() {
    ...
    // 2. Batch create 2 limit orders at extreme prices, via V2
    ...
    // 3. Batch cancel all created orders (cleanup), via V2
    let cancel_resp = tokio::time::timeout(common::TEST_TIMEOUT, async {
        client
            .batch_cancel_orders_v2(BatchCancelOrdersV2Request {
                orders: order_ids
                    .iter()
                    .map(|order_id| BatchCancelOrderV2RequestOrder {
...
async fn test_cancel_all_orders() {
    ...
    // 2. Create a resting order that cancel_all_orders should sweep up
    let create_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .create_order_v2(CreateOrderV2Request {
    ...
    // 3. Cancel all resting orders for the primary account
    tokio::time::timeout(common::TEST_TIMEOUT, async {
        client.cancel_all_orders(None).await
    })
    .await
    .expect("timeout")
    .expect("cancel_all_orders failed");

    // 4. Verify the order no longer appears among resting orders
    let orders_resp = tokio::time::timeout(common::TEST_TIMEOUT, async {
        client
            .get_orders(GetOrdersParams {
                limit: Some(100),
    ...
    assert!(!orders_resp.orders.iter().any(|o| o.order_id == order_id));
}

#[tokio::test]
async fn test_update_order_group_limit_with_subaccount() {
    common::load_env();
    let auth = common::load_auth();
    let client = common::demo_auth_client(auth);

    // 1. Create an order group on the primary account (subaccount 0)
    let create_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
    ...
    // 2. Update the order group limit, explicitly passing the subaccount query param
    tokio::time::timeout(common::TEST_TIMEOUT, async {
        client
    ...
    assert_eq!(get_resp.contracts_limit, Some(200));

    // 4. Cleanup
    let _ = tokio::time::timeout(common::TEST_TIMEOUT, async {
        client
            .delete_order_group(
                &group_id,
                SubaccountQueryParams {
