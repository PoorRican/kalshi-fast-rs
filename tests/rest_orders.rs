...
fn limit_order_v2(ticker: &str, price: &str) -> CreateOrderV2Request {
...
async fn test_order_lifecycle() {
    ...
    // 2. Create a limit order at an extreme price so it won't fill (1 cent YES) via the V2
    // event-order endpoint (the legacy V1 mutation endpoints were retired by Kalshi in
    // 2026-06).
    let create_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .create_order_v2(limit_order_v2(&market_ticker, "0.01"))
            .await
    })
    .await
    .expect("timeout")
...
async fn test_batch_order_lifecycle() {
    ...
    // 2. Batch create 2 limit orders at extreme prices via the V2 event-order endpoint.
    let batch_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .batch_create_orders_v2(BatchCreateOrdersV2Request {
                orders: vec![
                    limit_order_v2(&market_ticker, "0.01"),
                    limit_order_v2(&market_ticker, "0.02"),
                ],
            })
            .await
    })
