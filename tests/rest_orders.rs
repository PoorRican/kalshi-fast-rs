...
fn extreme_buy_order(ticker: String, price: &str) -> CreateOrderV2Request {
...
async fn test_order_lifecycle() {
    ...
    let create_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .create_order_v2(extreme_buy_order(market_ticker.clone(), "0.01"))
            .await
    })
    .await
    .expect("timeout")
    ...
    let batch_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .batch_create_orders_v2(BatchCreateOrdersV2Request {
                orders: vec![
                    extreme_buy_order(market_ticker.clone(), "0.01"),
                    extreme_buy_order(market_ticker.clone(), "0.02"),
                ],
            })
            .await
    })
