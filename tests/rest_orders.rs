...
fn limit_order(ticker: &str, price: &str) -> CreateOrderV2Request {
...
async fn test_order_lifecycle() {
    ...
    let create_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .create_order_v2(limit_order(&market_ticker, "0.0100"))
            .await
    })
    .await
    .expect("timeout")
    ...
    let batch_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .batch_create_orders_v2(BatchCreateOrdersV2Request {
                orders: vec![
                    limit_order(&market_ticker, "0.0100"),
                    limit_order(&market_ticker, "0.0200"),
                ],
            })
            .await
    })
