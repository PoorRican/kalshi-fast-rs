...
use kalshi_fast::{
    ...
    GetMarketsParams, GetOrdersParams, MarketStatusQuery, SelfTradePreventionType, TimeInForce,
};
...
fn order_v2(ticker: String, price: &str) -> CreateOrderV2Request {
...
async fn test_order_lifecycle() {
    ...
    // 2. Create a limit order at an extreme price so it won't fill (1 cent)
    let create_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .create_order_v2(order_v2(market_ticker.clone(), "0.01"))
            .await
    })
    .await
    .expect("timeout")
    ...
    let batch_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .batch_create_orders_v2(BatchCreateOrdersV2Request {
                orders: vec![
                    order_v2(market_ticker.clone(), "0.01"),
                    order_v2(market_ticker.clone(), "0.02"),
                ],
            })
            .await
    })
