...
/// The legacy `/portfolio/orders` mutation endpoints (create/cancel/amend/decrease/batch)
/// were removed from the API; use the lower-cost V2 event-order endpoints instead
/// (`/portfolio/events/orders/*`). See `docs/spec-parity.md`.
#[tokio::test]
async fn test_order_lifecycle_v2() {
    ...
    // 2. Create a limit order at an extreme price so it won't fill (1 cent, bid side)
...
async fn test_batch_order_lifecycle_v2() {
    ...
    let new_order = |price: &str| CreateOrderV2Request {
        ticker: market_ticker.clone(),
        side: BookSide::Bid,
        count: "1".to_string(),
    ...
    let batch_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .batch_create_orders_v2(BatchCreateOrdersV2Request {
                orders: vec![new_order("0.01"), new_order("0.02")],
            })
            .await
    })
    .await
