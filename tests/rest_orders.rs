...
fn create_order_v2(ticker: String, price: &str) -> CreateOrderV2Request {
...
async fn test_order_lifecycle() {
    ...
    let create_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .create_order_v2(create_order_v2(market_ticker.clone(), "0.01"))
            .await
    })
    .await
    .expect("timeout")
    ...
    let batch_resp = tokio::time::timeout(LIFECYCLE_TIMEOUT, async {
        client
            .batch_create_orders_v2(BatchCreateOrdersV2Request {
                orders: vec![
                    create_order_v2(market_ticker.clone(), "0.01"),
                    create_order_v2(market_ticker.clone(), "0.02"),
                ],
            })
            .await
    })
