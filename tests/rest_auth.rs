...
async fn test_get_account_api_limits() {
    ...
    // Restructured 2026-06-06 (automated API rate-limit tiers): nested bucket
    // limits plus the grants array.
    ...
    for grant in &resp.grants {
        assert!(!grant.level.is_empty());
        assert!(!grant.exchange_instance.is_empty());
    }
}
