...
fn historical_params_serialize_correctly() {
    ...
    assert_eq!(orders_json["min_ts"], 1700000000);
...
fn get_weather_index_response_deserializes() {
    ...
    assert_eq!(resp.timeseries[0].v, Some(82.15));
...
fn get_weather_index_calibrations_response_deserializes() {
    let json = r#"{
        ...
        "calibrations": [{
            ...
            "stations": [
                ...
                {"station_id": "KMIA2M", "weight": 0.5, "offset_c": -0.1, "update_note": "updated"}
            ]
        }]
    }"#;
    ...
    let resp: kalshi_fast::GetWeatherIndexCalibrationsResponse =
        serde_json::from_str(json).unwrap();
    assert_eq!(resp.calibrations.len(), 1);
    assert_eq!(resp.calibrations[0].stations.len(), 2);
    assert_eq!(
        resp.calibrations[0].stations[1].update_note.as_deref(),
        Some("updated")
    );
}
