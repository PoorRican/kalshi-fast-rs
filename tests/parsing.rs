...
fn get_exchange_status_response_deserializes_per_index_breakdown() {
    ...
    let statuses = resp
        .exchange_index_statuses
        .expect("exchange_index_statuses present");
...
fn get_subaccount_transfers_response_deserializes() {
    ...
    assert_eq!(
        ...
        kalshi_fast::TransferType::Cash
    );
    assert_eq!(resp.cursor, Some("c1".into()));
}

#[test]
fn get_subaccount_transfers_response_deserializes_position_row() {
    let json = r#"{
        "subaccount_transfers": [
            {
                "transfer_id":"t2","from_subaccount":0,"to_subaccount":1,
                "amount_cents":0,"created_ts":1700000000,"exchange_index":0,
                "transfer_type":"position","market_ticker":"MKT-1","side":"yes",
                "count":5,"price":"0.5000"
            }
        ]
    }"#;
    ...
    assert_eq!(row.transfer_type, kalshi_fast::TransferType::Position);
    assert_eq!(row.market_ticker.as_deref(), Some("MKT-1"));
    assert_eq!(row.count, Some(5));
    assert_eq!(row.price.as_deref(), Some("0.5000"));
}

#[test]
fn apply_subaccount_position_transfer_request_serializes_correctly() {
    let req = kalshi_fast::ApplySubaccountPositionTransferRequest {
        client_transfer_id: "11111111-1111-1111-1111-111111111111".into(),
        from_subaccount: 0,
        to_subaccount: 1,
    ...
    let json = serde_json::to_value(&req).unwrap();
    assert_eq!(json["side"], "yes");
    assert_eq!(json["market_ticker"], "MKT-1");
