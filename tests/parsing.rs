...
fn create_order_v2_response_deserializes() {
    ...
    assert_eq!(resp.client_order_id.as_deref(), Some("client-1"));
}
...
fn get_account_api_usage_level_volume_progress_response_deserializes() {
    let json = r#"{
        "volume_progress": [{
            ...
            "goals": [
                {"level": "expert", "earn_volume_goal_fp": "2000.00", "keep_volume_goal_fp": "1500.00"}
            ]
        }]
    }"#;
    ...
    assert_eq!(resp.volume_progress[0].goals.len(), 1);
...
fn api_key_deserializes_with_subaccount() {
    ...
    assert_eq!(key.subaccount, Some(3));

    let unrestricted_json = r#"{
        "api_key_id": "key-2",
        "name": "full-access",
        "scopes": ["read", "write"]
    }"#;
    let unrestricted: kalshi_fast::ApiKey = serde_json::from_str(unrestricted_json).unwrap();
    assert!(unrestricted.subaccount.is_none());
}
...
#[test]
fn get_subaccount_balances_response_deserializes_per_index() {
    // One entry per exchange index for the same subaccount. Added 2026-07-02.
    let json = r#"{
        "subaccount_balances": [
