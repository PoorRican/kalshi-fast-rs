...
#[test]
fn series_exchange_index_deserializes() {
    let json = r#"{"series": {"ticker": "SERIES-1", "exchange_index": 0}}"#;
...
fn get_events_response_deserializes() {
    ...
    assert_eq!(resp.events[0].exchange_index, None);
}

#[test]
fn event_settlement_sources_and_cadence_deserialize() {
    let json = r#"{
        "events": [{
            "event_ticker": "EVT-1",
            "exchange_index": 0,
...
fn get_series_fee_changes_response_deserializes() {
    ...
    assert_eq!(resp.series_fee_change_arr[0].id, "fc-1");
    assert_eq!(resp.series_fee_change_arr[0].fee_multiplier, 1.5);
    assert_eq!(
...
/// The spec types `id` as string and `scheduled_ts` as a date-time string, but
/// both are read leniently so a numeric encoding from the exchange still parses.
#[test]
fn series_fee_change_tolerates_numeric_id_and_scheduled_ts() {
    let json = r#"{
        "series_fee_change_arr": [
    ...
    let resp: GetSeriesFeeChangesResponse = serde_json::from_str(json).unwrap();
    assert_eq!(resp.series_fee_change_arr[0].id, "1");
    assert_eq!(resp.series_fee_change_arr[0].scheduled_ts, "1700000000");
    assert_eq!(resp.series_fee_change_arr[0].fee_multiplier, 5.0);
}
