...
#[test]
fn get_order_response_deserializes() {
    let json = r#"{
        "order": {
            "order_id": "ord-123",
            "user_id": "user-1",
...
fn get_exchange_status_response_deserializes() {
    ...
    // New fields (2026-07-02) tolerate absence on older payloads.
    assert!(!resp.intra_exchange_transfers_active);
    assert!(resp.exchange_index_statuses.is_empty());

    let with_shards_json = r#"{
        "exchange_active": true,
        "trading_active": true,
    ...
    let resp: GetExchangeStatusResponse = serde_json::from_str(with_shards_json).unwrap();
    assert!(resp.intra_exchange_transfers_active);
...
fn get_subaccount_balances_response_deserializes() {
    ...
    // A subaccount with funds on multiple exchange indexes returns one entry
    // per index (2026-07-02).
    let multi_index_json = r#"{
        "subaccount_balances": [
            {"subaccount_number":1,"balance":100,"updated_ts":1700000000,"exchange_index":0},
            {"subaccount_number":1,"balance":50,"updated_ts":1700000000,"exchange_index":1}
        ]
    }"#;
    let resp: GetSubaccountBalancesResponse = serde_json::from_str(multi_index_json).unwrap();
