...
fn get_subaccount_balances_response_deserializes() {
    let json = r#"{
        "subaccount_balances": [
            {"subaccount_number":1,"balance":100,"updated_ts":1700000000},
            {"subaccount_number":1,"exchange_index":2,"balance":50,"updated_ts":1700000001}
        ]
    }"#;
    ...
    // exchange_index defaults to 0 when absent (pre-rollout payloads).
    assert_eq!(resp.subaccount_balances[0].exchange_index, 0);
    assert_eq!(resp.subaccount_balances[1].exchange_index, 2);
}
