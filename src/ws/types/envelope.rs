    ...
    fn ws_envelope_into_message_pyth_value() {
        ...
        match msg {
            WsMessageV2::Data(WsDataMessageV2::PythValue { msg, .. }) => {
                assert_eq!(msg.underlying_ticker, "Crypto.BTC/USD");
                assert_eq!(msg.value_usd, "65000.12340000");
                assert_eq!(msg.source_ts_ms, 1704067200000);
                assert_eq!(msg.received_at, 1704067200050);
            }
        ...
        match msg {
            WsMessageV2::Data(WsDataMessageV2::PythValueUnderlyingList { msg, .. }) => {
                assert_eq!(
                    ...
                    vec!["Crypto.BTC/USD".to_string(), "Crypto.ETH/USD".to_string()]
                );
            }
