...
mod tests {
    ...
    #[test]
    fn ws_message_from_bytes_parses_pyth_value() {
        let json = r#"{
            "type":"pyth_value",
            "sid":42,
            "seq":1,
            "msg":{
                ...
                "value_usd":"65432.10000000",
                "source_ts_ms":1715793680789,
                "received_at":1715793680900
            }
        }"#;
        let msg = WsMessageV2::from_bytes(json.as_bytes()).unwrap();
        match msg {
            WsMessageV2::Data(WsDataMessageV2::PythValue { sid, msg, .. }) => {
        ...
        let raw = WsRawEvent::new(Bytes::from(json));
        let borrowed = raw.parse_borrowed().unwrap();
        assert!(matches!(
            borrowed,
            WsMessageRef::Data(WsDataMessageRef::PythValue { .. })
        ));
    }

    #[test]
    fn ws_message_from_bytes_parses_pyth_value_underlying_list() {
        let json = r#"{
