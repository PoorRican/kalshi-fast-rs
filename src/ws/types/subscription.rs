...
pub enum WsUpdateAction {
    ...
    /// Request the underlying tickers observed on the Pyth stream in the last
    /// two hours, without modifying the subscription (server replies with a
...
mod tests {
    ...
    #[test]
    fn validate_update_index_and_underlying_actions_are_mutually_exclusive() {
        // An index action with underlying_tickers set is rejected, and vice versa.
        let params = WsUpdateSubscriptionParamsV2 {
            action: WsUpdateAction::SubscribeIndices,
            sid: Some(1),
            sids: None,
