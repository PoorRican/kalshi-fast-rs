...
pub struct WsSubscriptionParamsV2 {
    ...
    /// Pyth underlying tickers to seed for `pyth_value` subscriptions.
    /// Use `["all"]` to track every available underlying. Added 2026-07-23.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct WsUpdateSubscriptionParamsV2 {
    ...
    /// `subscribe_underlyings` / `unsubscribe_underlyings` actions; use `["all"]` to track every
    /// available underlying. Added 2026-07-23.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub underlying_tickers: Option<Vec<String>>,
}
...
pub enum WsUpdateAction {
    ...
    Indexlist,
    /// Add the supplied `underlying_tickers` to a `pyth_value` subscription. Added 2026-07-23.
    SubscribeUnderlyings,
    /// Remove the supplied `underlying_tickers` from a `pyth_value` subscription. Added
    /// 2026-07-23.
    UnsubscribeUnderlyings,
    /// Request recently streamed Pyth underlying tickers without modifying the subscription
    /// (server replies with a `pyth_value_underlying_list`). Added 2026-07-23.
    UnderlyingList,
}
...
impl WsUpdateAction {
    ...
    /// Whether this action operates on Pyth underlying tickers rather than market targets.
...
pub(crate) fn validate_update(params: &WsUpdateSubscriptionParamsV2) -> Result<(), KalshiError> {
    ...
    // Pyth underlying actions are mutually exclusive with market targets and have their own
    // `underlying_tickers` requirements.
...
mod tests {
    ...
    fn validate_update_subscribe_underlyings_requires_underlying_tickers() {
        ...
        let ok = WsUpdateSubscriptionParamsV2 {
            ...
            skip_ticker_ack: None,
            index_ids: None,
            underlying_tickers: Some(vec!["BTCUSD".to_string()]),
        };
        assert!(validate_update(&ok).is_ok());
    }
