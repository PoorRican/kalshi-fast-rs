...
pub struct WsSubscriptionParamsV2 {
    ...
    /// Pyth underlying tickers to seed on a `pyth_value` subscription. Omit to
    /// subscribe with no underlyings and add them later; use `["all"]` to track
    /// every available underlying. Added 2026-07-23.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct WsUpdateSubscriptionParamsV2 {
    ...
    /// `subscribe_underlyings` / `unsubscribe_underlyings` actions; use `["all"]`
    /// to track every available underlying. Added 2026-07-23.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub underlying_tickers: Option<Vec<String>>,
}
...
pub enum WsUpdateAction {
    ...
    /// Request the underlying tickers observed on the Pyth stream in the last two
    /// hours (server replies with a `pyth_value_underlying_list`).
    UnderlyingList,
}
...
pub(crate) fn validate_update(params: &WsUpdateSubscriptionParamsV2) -> Result<(), KalshiError> {
    ...
    // Pyth underlying actions are mutually exclusive with market targets and have
    // their own `underlying_tickers` requirements.
