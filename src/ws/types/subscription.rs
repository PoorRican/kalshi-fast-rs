...
pub struct WsSubscriptionParamsV2 {
    ...
    /// Omit to subscribe with no underlyings and add them later via
    /// `subscribe_underlyings`; use `["all"]` to track every available underlying.
    /// Added 2026-07-23.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct WsUpdateSubscriptionParamsV2 {
    ...
    /// `subscribe_underlyings` / `unsubscribe_underlyings` actions. Added 2026-07-23.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub underlying_tickers: Option<Vec<String>>,
}
...
pub enum WsUpdateAction {
    ...
    /// Remove the supplied `underlying_tickers` from a `pyth_value` subscription. Added 2026-07-23.
    UnsubscribeUnderlyings,
    /// Request the underlying tickers recently observed on the Pyth stream
    /// without modifying the subscription (server replies with a
...
pub(crate) fn validate_update(params: &WsUpdateSubscriptionParamsV2) -> Result<(), KalshiError> {
    ...
    // Pyth underlying-ticker actions are mutually exclusive with market targets and
