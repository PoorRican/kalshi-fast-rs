...
pub struct WsSubscriptionParamsV2 {
    ...
    /// Orderbook channels only. When `true`, no-side `orderbook_delta` /
    /// `orderbook_snapshot` updates report in yes-leg pricing instead of
    /// no-leg pricing. Defaults to `false` (legacy no-leg pricing) upstream;
    /// Kalshi has announced the default will flip to `true` in a future
    /// release, after which the flag will be removed and unified yes-leg
    /// pricing becomes the only behavior.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub use_yes_price: Option<bool>,
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Pyth underlying tickers for `pyth_value` subscriptions. Omit to
    /// subscribe with no underlyings and add them later; use `["all"]` to
    /// track every available underlying.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub enum WsUpdateAction {
    ...
    /// Request the recently streamed Pyth underlying tickers without
    /// modifying the subscription (server replies with a
...
pub(crate) fn validate_update(params: &WsUpdateSubscriptionParamsV2) -> Result<(), KalshiError> {
    ...
    if params.action.is_index_action() {
        if has_any_market_tickers || has_any_market_ids || has_underlying_tickers {
            return Err(KalshiError::InvalidParams(
                "update_subscription: index actions do not support market_ticker(s), \
                 market_id(s), or underlying_tickers"
                    .to_string(),
            ));
        }
        if matches!(
    ...
    if params.action.is_underlying_action() {
        if has_any_market_tickers || has_any_market_ids || has_index_ids {
            return Err(KalshiError::InvalidParams(
                "update_subscription: underlying actions do not support market_ticker(s), \
                 market_id(s), or index_ids"
                    .to_string(),
            ));
        }
        if matches!(
...
pub(crate) fn validate_subscription(params: &WsSubscriptionParamsV2) -> Result<(), KalshiError> {
    ...
    if params.use_yes_price.is_some() && !has_orderbook_delta {
        return Err(KalshiError::InvalidParams(
            "subscribe: use_yes_price only allowed for orderbook_delta".to_string(),
        ));
    }
...
mod tests {
    ...
    #[test]
    fn validate_subscription_use_yes_price_only_for_orderbook_delta() {
        let params = WsSubscriptionParamsV2 {
            channels: vec![WsChannelV2::Ticker],
    ...
    fn validate_update_subscribe_underlyings_requires_underlying_tickers() {
        let missing = WsUpdateSubscriptionParamsV2 {
            action: WsUpdateAction::SubscribeUnderlyings,
            sid: Some(1),
            sids: None,
            market_ticker: None,
            market_tickers: None,
    ...
    fn validate_update_underlying_list_allows_empty_underlying_tickers() {
        let params = WsUpdateSubscriptionParamsV2 {
            action: WsUpdateAction::UnderlyingList,
            sid: Some(1),
            sids: None,
            market_ticker: None,
            market_tickers: None,
    ...
    #[test]
    fn validate_update_underlying_and_index_actions_are_mutually_exclusive() {
        let params = WsUpdateSubscriptionParamsV2 {
            action: WsUpdateAction::SubscribeUnderlyings,
            sid: Some(1),
            sids: None,
