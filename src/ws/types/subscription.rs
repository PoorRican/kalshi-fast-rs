...
pub struct WsSubscriptionParamsV2 {
    ...
    /// track every available underlying. Added 2026-07-23.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub enum WsUpdateAction {
    ...
    /// Request the underlying tickers observed on the Pyth stream in the
    /// last two hours, without modifying the subscription (server replies
    /// with a `pyth_value_underlying_list`). Added 2026-07-23.
    UnderlyingList,
}
...
pub(crate) fn validate_update(params: &WsUpdateSubscriptionParamsV2) -> Result<(), KalshiError> {
    ...
    // and have their own `underlying_tickers` requirements.
...
mod tests {
    ...
    fn validate_update_subscribe_underlyings_requires_underlying_tickers() {
        ...
        let ok = WsUpdateSubscriptionParamsV2 {
            ...
            skip_ticker_ack: None,
            index_ids: None,
            underlying_tickers: Some(vec!["KXBTC".to_string()]),
        };
        assert!(validate_update(&ok).is_ok());
    }
