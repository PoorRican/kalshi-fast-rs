...
impl WsWireMessage {
    pub(super) fn into_message(self) -> WsMessageV2 {
        match self {
            ...
            WsWireMessage::PythUnderlyingList { sid, seq, msg } => {
...
impl<'a> WsWireMessageRef<'a> {
    pub(super) fn into_message(self) -> WsMessageRef<'a> {
        match self {
            ...
            WsWireMessageRef::PythUnderlyingList { sid, seq, msg } => {
                WsMessageRef::Data(WsDataMessageRef::PythUnderlyingList { sid, seq, msg })
            }
        }
    }
}
