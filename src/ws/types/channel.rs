...
pub enum WsChannelV2 {
    ...
    CfbenchmarksValue,
    /// Deduplicated real-time Pyth prices by underlying ticker. Requires
...
mod tests {
    ...
    #[test]
    fn removed_multivariate_channel_stays_removed() {
        // The bare "multivariate" channel (message type "multivariate_lookup")
        // was removed from the live AsyncAPI enum 2026-08-06. Subscribing to it
        // now returns an unknown-channel error from the exchange, so unknown
        // channel strings must not deserialize into a known variant.
        let result: Result<WsChannelV2, _> = serde_json::from_str("\"multivariate\"");
