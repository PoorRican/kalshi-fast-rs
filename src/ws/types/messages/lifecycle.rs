...
pub struct WsMarketLifecycleV2 {
    ...
    /// Identifier for the exchange shard the market lives on. Only present when the market is
    ...
    /// Valid price bands in fixed-point dollars, emitted alongside `price_level_structure` on
    ...
    /// Top-level updated custom strike; present only on `metadata_updated` events for
    /// custom/structured markets. Added 2026-06-18.
    #[serde(default)]
