...
pub struct WsMarketLifecycleV2 {
    ...
    /// Added 2026-06-18, alongside `cap_strike`.
    #[serde(default)]
    ...
    /// Top-level custom strike, present only on `metadata_updated` events for
...
/// A valid order price band (`{ start, end, step }`, in fixed-point dollars).
/// Emitted on `market_lifecycle_v2` `created` / `price_level_structure_updated`
/// events alongside `price_level_structure`.
#[derive(Debug, Clone, Deserialize)]
