...
pub struct WsMarketLifecycleV2 {
    ...
    /// this key exists **only** when the market is created.
    #[serde(default)]
    ...
    /// `price_level_structure` on market creation and `price_level_structure_updated`
    ...
    /// exists **only** on `metadata_updated` events with a custom strike type.
    #[serde(default)]
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    /// `metadata_updated` events with a custom strike type.
    #[serde(default)]
    pub custom_strike: Option<BTreeMap<String, String>>,
