...
mod tests {
    ...
    /// `price_ranges` is emitted alongside `price_level_structure` on market
    /// creation and `price_level_structure_updated` events. Added 2026-06-30.
    #[test]
    fn price_ranges_parse_on_created_event() {
        let json = r#"{
            "event_type": "created",
            "market_ticker": "KXHIGHNY-24JAN01-T60",
        ...
        let owned: WsMarketLifecycleV2 = serde_json::from_str(json).unwrap();
        let ranges = owned.price_ranges.unwrap();
