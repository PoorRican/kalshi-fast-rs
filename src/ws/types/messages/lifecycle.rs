...
pub struct WsMarketLifecycleV2 {
    ...
    /// Exchange shard the market lives on. Present only when the market is
    ...
    /// How `floor_strike` / `cap_strike` are interpreted (e.g. `"between"`
    /// uses both, `"greater"` uses `floor_strike` only, `"less"` uses
    /// `cap_strike` only). Present only on `metadata_updated` events.
    /// Added 2026-06-18.
    #[serde(default)]
    ...
    /// Present only on `metadata_updated` events with a custom or structured
...
mod tests {
    ...
    /// `strike_type` / `cap_strike` / `custom_strike` are additional
    /// `metadata_updated`-only fields (2026-06-18) alongside `exchange_index`
    /// (created-only, 2026-07-30) and `price_ranges` (created /
    /// `price_level_structure_updated`, 2026-07-02).
    #[test]
    fn metadata_updated_surfaces_strike_range_fields() {
        let json = r#"{
            "event_type": "metadata_updated",
            "market_ticker": "KXHIGHNY-24JAN01-T60",
            "strike_type": "between",
    ...
    fn event_lifecycle_surfaces_exchange_index() {
        ...
        assert_eq!(owned.exchange_index, Some(1));

        let borrowed: WsEventLifecycleRef = serde_json::from_str(json).unwrap();
        assert_eq!(borrowed.into_owned().exchange_index, Some(1));
    }
}
