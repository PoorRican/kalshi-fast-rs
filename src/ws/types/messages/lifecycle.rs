...
pub struct WsMarketLifecycleV2 {
    ...
    /// events for custom/structured strike types. Added 2026-06-18.
    #[serde(default)]
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    /// Valid price bands for the market; present on `created` and
    ...
    pub cap_strike: Option<f64>,
    /// Top-level updated custom strike; present only on `metadata_updated`
    /// events for custom/structured strike types.
    #[serde(default)]
    pub custom_strike: Option<BTreeMap<String, String>>,
...
mod tests {
    ...
    /// Added 2026-06-18: `metadata_updated` also carries `strike_type` and
    /// `cap_strike` (plus `custom_strike` for custom/structured markets)
    /// alongside `floor_strike`, and `created` / `price_level_structure_updated`
    /// events carry `price_ranges`.
    #[test]
