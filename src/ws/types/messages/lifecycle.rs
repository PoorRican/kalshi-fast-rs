...
pub struct WsMarketLifecycleV2 {
    ...
    /// AsyncAPI this key will ONLY exist when the market is created.
    /// Added 2026-07-30.
    #[serde(default)]
    ...
    /// AsyncAPI this is emitted alongside `price_level_structure` (on market
    /// creation and `price_level_structure_updated` events). Added 2026-07-02.
    #[serde(default)]
    ...
    /// `additional_metadata.strike_type` (which is emitted on market creation).
    /// Added 2026-06-18.
    #[serde(default)]
    ...
    /// on `metadata_updated` events and is distinct from
    /// `additional_metadata.cap_strike` (which is emitted on market creation).
    /// Added 2026-06-18.
    #[serde(default)]
    pub cap_strike: Option<f64>,
    /// Top-level updated custom strike, for custom/structured markets. Per the
    /// AsyncAPI this key exists **only** on `metadata_updated` events and is
    /// distinct from `additional_metadata.custom_strike` (which is emitted on
    /// market creation). Added 2026-06-18.
    #[serde(default)]
...
pub struct WsEventLifecycle {
    ...
    /// Identifier for the exchange shard the event's markets live on. Spec
    /// marks this required, but modeled as `Option` (consistent with the
    /// other "required" fields on this struct) and defensively defaulted so
    /// payloads predating the flag still parse. Added 2026-07-30.
    #[serde(default)]
    pub exchange_index: Option<i64>,
    #[serde(default)]
    pub title: Option<String>,
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    /// alongside `price_level_structure`.
    #[serde(default)]
    pub price_ranges: Option<Vec<PriceRange>>,
    ...
    /// only on `metadata_updated` events.
    #[serde(default)]
    pub custom_strike: Option<BTreeMap<String, String>>,
...
mod tests {
    ...
    /// (`floor_strike`, `yes_sub_title`, `strike_type`, `cap_strike`,
    /// `custom_strike`) at the top level of the payload, not nested under
    /// `additional_metadata`. They must not be silently dropped.
    #[test]
    ...
    /// `created` events carry `exchange_index` (identifying the exchange
    /// shard the market lives on) and, alongside `price_level_structure`,
    /// `price_ranges` (the valid price bands for the market). Added
    /// 2026-07-30 and 2026-07-02 respectively.
    #[test]
    fn created_event_surfaces_exchange_index_and_price_ranges() {
        let json = r#"{
            "event_type": "created",
        ...
        let price_ranges = owned.price_ranges.as_ref().expect("price_ranges present");
        assert_eq!(price_ranges.len(), 2);
        assert_eq!(price_ranges[0].start, "0.01");
        assert_eq!(price_ranges[0].end, "0.50");
        assert_eq!(price_ranges[0].step, "0.01");

        // Borrowed path must round-trip to the same values.
        let borrowed: WsMarketLifecycleV2Ref = serde_json::from_str(json).unwrap();
        let round_tripped = borrowed.into_owned();
        assert_eq!(round_tripped.exchange_index, Some(0));
        let price_ranges = round_tripped
            .price_ranges
            .as_ref()
            .expect("price_ranges present");
        assert_eq!(price_ranges.len(), 2);
        assert_eq!(price_ranges[1].start, "0.50");
        assert_eq!(price_ranges[1].end, "0.99");
        assert_eq!(price_ranges[1].step, "0.01");

        // Payloads predating these fields must still parse.
        let without_fields = r#"{
            "event_type": "created",
            "market_ticker": "INXD-23SEP14-B4487"
        }"#;
        let owned: WsMarketLifecycleV2 = serde_json::from_str(without_fields).unwrap();
        assert_eq!(owned.exchange_index, None);
        assert!(owned.price_ranges.is_none());
    }

    /// `event_lifecycle` messages carry `exchange_index`, identifying the
    /// exchange shard the event's markets live on. Spec marks it required,
    /// but modeled as `Option` (consistent with the other "required" fields
    /// on this struct) and defensively defaulted. Added 2026-07-30.
    #[test]
    fn event_lifecycle_surfaces_exchange_index() {
        ...
        assert_eq!(borrowed.into_owned().exchange_index, Some(0));

        let without_field = r#"{"event_ticker": "KXQUICKSETTLE-26JAN25H2150"}"#;
        let owned: WsEventLifecycle = serde_json::from_str(without_field).unwrap();
        assert_eq!(owned.exchange_index, None);
    }
}
