...
/// A valid price band, in fixed-point dollars, within a [`WsMarketLifecycleV2`]
/// `price_ranges` array. Mirrors the REST `PriceRange` shape but kept local so
/// `ws::types` has no dependency on `rest::types`.
...
pub struct WsMarketLifecycleV2 {
    ...
    /// Exchange shard the market lives on. Only present on `created` events.
    /// Added 2026-07-30.
    #[serde(default)]
    ...
    /// on `created` and `price_level_structure_updated` events. Added 2026-07-02.
    #[serde(default)]
    ...
    /// Only present on `metadata_updated` events with a custom/structured strike
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    #[serde(default)]
    pub cap_strike: Option<f64>,
    /// Present only on `metadata_updated` events with a custom/structured strike
    /// type.
    #[serde(default)]
    pub custom_strike: Option<BTreeMap<String, String>>,
    #[serde(default, borrow)]
