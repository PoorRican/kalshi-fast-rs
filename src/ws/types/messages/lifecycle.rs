...
pub struct WsMarketLifecycleV2 {
    ...
    /// Added to the AsyncAPI on 2026-06-17.
    #[serde(default)]
    pub cap_strike: Option<f64>,
    /// Determines how `floor_strike`/`cap_strike` are interpreted (e.g. `"between"`,
    /// `"greater"`). Present only on `metadata_updated` events. Added 2026-06-17.
    #[serde(default)]
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    #[serde(default)]
    pub cap_strike: Option<f64>,
    /// Determines how `floor_strike`/`cap_strike` are interpreted; present only on
