...
pub struct WsMarketLifecycleV2 {
    ...
    /// alongside `price_level_structure` on market creation and
    /// `price_level_structure_updated` events. Added 2026-07-02.
    #[serde(default)]
    pub price_ranges: Option<Vec<crate::rest::PriceRange>>,
    /// Exchange shard the market lives on. Only present on market creation.
    /// Added 2026-07-30.
    #[serde(default)]
    ...
    /// Top-level updated cap (upper bound) strike. `metadata_updated` only.
    /// Added 2026-06-18, alongside `floor_strike`.
    #[serde(default)]
    pub cap_strike: Option<f64>,
    /// Top-level updated strike type (e.g. `"between"`, `"greater"`, `"less"`),
    /// determining how `floor_strike` / `cap_strike` are interpreted.
    /// `metadata_updated` only. Added 2026-06-18.
    #[serde(default)]
    pub strike_type: Option<String>,
    /// Top-level updated custom/structured strike. `metadata_updated` only,
    /// and only for custom or structured strike types. Added 2026-06-18.
    #[serde(default)]
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    /// on market creation and `price_level_structure_updated` events.
    #[serde(default)]
    pub price_ranges: Option<Vec<crate::rest::PriceRange>>,
    /// Exchange shard the market lives on. Only present on market creation.
    #[serde(default)]
