...
pub struct WsMarketLifecycleV2 {
    ...
    /// Valid price bands for the market, in fixed-point dollars. Optional —
    /// emitted alongside `price_level_structure` (on market creation and
    /// `price_level_structure_updated` events).
    #[serde(default)]
    pub price_ranges: Option<Vec<WsMarketPriceRange>>,
    ...
    /// Determines how `floor_strike` / `cap_strike` are interpreted. Per the
    ...
    /// Identifier for the exchange shard the market lives on. Optional — this
    /// key will ONLY exist when the market is created.
    #[serde(default)]
...
/// A valid price band for a market, in fixed-point dollars. Emitted alongside
/// `price_level_structure` on `market_lifecycle_v2` `created` and
/// `price_level_structure_updated` events.
#[derive(Debug, Clone, Deserialize)]
pub struct WsMarketPriceRange {
...
pub struct WsEventLifecycle {
    ...
    /// Identifier for the exchange shard the event's markets live on. Marked
    /// required by the AsyncAPI, but modeled `Option` for lenient parsing
    /// consistent with the rest of this struct's fields.
    #[serde(default)]
    pub exchange_index: Option<i64>,
    #[serde(default)]
    pub title: Option<String>,
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    /// Valid price bands for the market; present alongside
    /// `price_level_structure`.
    #[serde(default)]
    pub price_ranges: Option<Vec<WsMarketPriceRangeRef<'a>>>,
...
/// Borrowed version of [`WsMarketPriceRange`].
#[derive(Debug, Clone, Deserialize)]
pub struct WsMarketPriceRangeRef<'a> {
    #[serde(borrow)]
    pub start: Cow<'a, str>,
    #[serde(borrow)]
    pub end: Cow<'a, str>,
...
impl<'a> WsMarketPriceRangeRef<'a> {
    pub fn into_owned(self) -> WsMarketPriceRange {
        WsMarketPriceRange {
    ...
    pub fn into_owned(self) -> WsMarketLifecycleV2 {
        WsMarketLifecycleV2 {
            ...
            price_ranges: self.price_ranges.map(|ranges| {
                ranges
                    .into_iter()
                    .map(WsMarketPriceRangeRef::into_owned)
                    .collect()
            }),
