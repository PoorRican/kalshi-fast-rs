...
pub struct WsMarketLifecycleV2 {
    ...
    /// this key exists **only** on `created` events. Added 2026-07-30.
    #[serde(default)]
...
pub struct WsEventLifecycle {
    ...
    /// Identifier for the exchange shard the event's markets live on. Required
    /// by the AsyncAPI as of 2026-07-30.
    #[serde(default)]
    pub exchange_index: Option<u32>,
    #[serde(default)]
    pub title: Option<String>,
...
mod tests {
    ...
    fn metadata_updated_surfaces_top_level_fields() {
        ...
        assert_eq!(owned.strike_type.as_deref(), Some("greater"));
        ...
        assert_eq!(round_tripped.strike_type.as_deref(), Some("greater"));
    ...
    /// `created` events carry `exchange_index` and `price_ranges` at the top
    /// level, alongside `price_level_structure`.
    #[test]
