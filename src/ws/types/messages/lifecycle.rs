...
pub struct WsMarketLifecycleV2 {
    ...
    /// Top-level custom/structured strike; present only on `metadata_updated` events for
    /// custom-strike markets. Added 2026-06-18.
    #[serde(default)]
...
mod tests {
    ...
    /// Per the AsyncAPI (added 2026-06-18), `metadata_updated` also carries
    /// `strike_type`, `cap_strike`, and `custom_strike` at the top level,
    /// alongside `floor_strike`.
    #[test]
    fn metadata_updated_surfaces_strike_fields() {
        ...
        assert_eq!(owned.floor_strike, Some(60.5));
        assert_eq!(owned.cap_strike, Some(70.0));
        assert_eq!(
        ...
        assert_eq!(round_tripped.cap_strike, Some(70.0));
    }

    /// Per the AsyncAPI (added 2026-07-02), `price_ranges` is emitted alongside
