...
pub struct WsMarketLifecycleV2 {
    ...
    /// AsyncAPI this key exists **only** on `created` events. Added 2026-07-30.
    #[serde(default)]
    ...
    /// The market's valid price bands, in fixed-point dollars. Emitted
...
pub struct WsEventLifecycle {
    ...
    /// marks this required; modeled as `Option` for the same resilience
    /// reasons as the other fields here. Added 2026-07-30.
    #[serde(default)]
    pub exchange_index: Option<i64>,
    #[serde(default)]
    pub title: Option<String>,
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    #[serde(default)]
    pub event_type: Option<WsMarketLifecycleEventType>,
    /// Present only on `created` events.
    #[serde(default)]
    pub exchange_index: Option<i64>,
    #[serde(default)]
    pub open_ts: Option<i64>,
    ...
    /// Present on `created` and `price_level_structure_updated` events.
    #[serde(default)]
    pub price_ranges: Option<Vec<PriceRange>>,
