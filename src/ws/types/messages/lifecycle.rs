...
pub struct WsMarketLifecycleV2 {
    ...
    /// Top-level custom strike (custom/structured markets only), present only
...
mod tests {
    ...
    /// `custom_strike` at the top level (added 2026-06-18), distinct from the
    /// `additional_metadata.*` copies emitted on creation.
    #[test]
    fn metadata_updated_surfaces_strike_fields() {
        ...
        assert_eq!(
            owned.custom_strike.as_ref().and_then(|m| m.get("target-a")),
            Some(&"yes".to_string())
        );
    ...
    /// `price_level_structure_updated` carries `price_ranges` alongside
    /// `price_level_structure` (added 2026-06-30).
    #[test]
