...
pub struct WsMarketLifecycleV2 {
    ...
    /// Top-level cap strike. Only present on `metadata_updated` events. Added 2026-06-17.
    #[serde(default)]
    pub cap_strike: Option<f64>,
    /// How floor_strike / cap_strike are interpreted (`"between"`, `"greater"`, `"less"`).
    /// Only present on `metadata_updated` events. Added 2026-06-17.
    #[serde(default)]
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    #[serde(default)]
    pub cap_strike: Option<f64>,
    /// How floor_strike / cap_strike are interpreted; present only on `metadata_updated` events. Added 2026-06-17.
    #[serde(default, borrow)]
