...
pub struct WsMarketLifecycleV2 {
    ...
    /// Identifier for the exchange shard the market lives on. Only present when
    /// the market is created. Added 2026-07-30.
    #[serde(default)]
    ...
    /// Removed from the AsyncAPI schema; no `fractional_trading_updated` event
    /// type is emitted anymore now that fractional trading has fully rolled out.
    #[deprecated(since = "0.7.0", note = "removed from the AsyncAPI schema")]
    #[serde(default)]
    ...
    /// How `floor_strike`/`cap_strike` are interpreted (e.g. "between" uses both,
    /// "greater" uses floor_strike only, "less" uses cap_strike only). Present
    ...
    /// events with a custom/structured strike type. Added 2026-06-18.
    #[serde(default)]
...
pub enum WsMarketLifecycleEventType {
    ...
    Determined,
    Settled,
    /// Removed from the AsyncAPI `event_type` enum now that fractional trading
    /// has fully rolled out; the exchange no longer emits this event type.
    #[deprecated(since = "0.7.0", note = "removed from the AsyncAPI schema")]
...
pub struct WsEventLifecycle {
    ...
    /// required by the AsyncAPI as of 2026-07-30, but kept `Option` since older
    /// cached payloads may predate the field.
    #[serde(default)]
    pub exchange_index: Option<i64>,
    #[serde(default)]
    pub title: Option<String>,
