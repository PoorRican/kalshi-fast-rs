...
pub struct WsMarketLifecycleV2 {
    ...
    /// Identifies the exchange shard the market lives on. Only present when
...
/// A valid price band for a market, in fixed-point dollars. Added 2026-07-02.
#[derive(Debug, Clone, Deserialize)]
...
pub struct WsEventLifecycle {
    ...
    /// Identifies the exchange shard the event's markets live on. Added 2026-07-30.
    #[serde(default)]
    pub exchange_index: Option<i64>,
    #[serde(default)]
    pub title: Option<String>,
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    /// Valid price bands for the market, in fixed-point dollars. Added 2026-07-02.
    #[serde(default, borrow)]
    pub price_ranges: Option<Vec<WsPriceRangeRef<'a>>>,
    /// Identifies the exchange shard the market lives on. Added 2026-07-30.
    #[serde(default)]
    pub exchange_index: Option<i64>,
...
impl<'a> WsMarketLifecycleV2Ref<'a> {
    pub fn into_owned(self) -> WsMarketLifecycleV2 {
        WsMarketLifecycleV2 {
            ...
            price_ranges: self
                .price_ranges
                .map(|v| v.into_iter().map(WsPriceRangeRef::into_owned).collect()),
