...
pub struct WsMarketLifecycleV2 {
    ...
    /// Added to AsyncAPI 2026-06-17.
    #[serde(default)]
    pub cap_strike: Option<f64>,
    /// How floor/cap strike are interpreted (`"between"`, `"greater"`, `"less"`).
    /// Present only on `metadata_updated` events. Added to AsyncAPI 2026-06-17.
    #[serde(default)]
    pub strike_type: Option<String>,
    /// Custom or structured strike definition; present only on `metadata_updated`
    /// events with a custom/structured strike type. Added to AsyncAPI 2026-06-17.
    #[serde(default)]
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    #[serde(default)]
    pub floor_strike: Option<f64>,
    /// Top-level cap strike; present only on `metadata_updated` events.
    #[serde(default)]
    pub cap_strike: Option<f64>,
    /// Strike type interpretation; present only on `metadata_updated` events.
    #[serde(default, borrow)]
    pub strike_type: Option<Cow<'a, str>>,
    /// Custom strike definition; present only on `metadata_updated` events.
    #[serde(default)]
    pub custom_strike: Option<Map<String, Value>>,
...
mod tests {
    ...
    /// top level of the payload. Since 2026-06-17, `strike_type`, `cap_strike`,
    /// and `custom_strike` join `floor_strike` / `yes_sub_title` at the top level.
    #[test]
