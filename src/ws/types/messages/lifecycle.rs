...
pub struct WsMarketLifecycleV2 {
    ...
    /// The valid price bands for the market, in fixed-point dollars. Emitted
    /// alongside `price_level_structure` (on market creation and
    /// `price_level_structure_updated` events). Added 2026-07-23.
    #[serde(default)]
...
pub struct WsMarketLifecycleV2Ref<'a> {
    ...
    /// The valid price bands for the market, in fixed-point dollars. Added 2026-07-23.
    #[serde(default)]
    pub price_ranges: Option<Vec<PriceRange>>,
...
mod tests {
    ...
    /// Per the AsyncAPI, `price_level_structure_updated` carries `price_ranges`
    /// alongside `price_level_structure`. Added 2026-07-23.
    #[test]
    fn price_level_structure_updated_surfaces_price_ranges() {
        ...
        assert_eq!(
            owned.event_type,
            Some(WsMarketLifecycleEventType::PriceLevelStructureUpdated)
        );
