...
pub struct WsMarketLifecycleV2 {
    ...
    /// Identifier for the exchange shard the market lives on. Present only when the market is
    /// created. Added 2026-07-30.
    #[serde(default)]
    ...
    /// No longer present in the AsyncAPI `marketLifecycleV2Payload` schema or its `event_type`
    /// enum (the `fractional_trading_updated` event type is gone). Kept for lossless round-trip
    /// of any historical/cached payloads that still carry it.
    #[deprecated(
        since = "0.7.0",
        note = "removed from marketLifecycleV2Payload upstream; fractional_trading_updated no longer fires"
    )]
    #[serde(default)]
    pub fractional_trading_enabled: Option<bool>,
    #[serde(default)]
    pub price_level_structure: Option<String>,
    /// Valid price bands for the market, in fixed-point dollars. Emitted alongside
    /// `price_level_structure` on `created` and `price_level_structure_updated` events. Added
    /// 2026-07-02.
    #[serde(default)]
    ...
    /// Top-level updated strike type (`between`/`greater`/`less`/etc). Present only on
    /// `metadata_updated` events, distinct from `additional_metadata.strike_type` (emitted on
    /// market creation). Added 2026-06-18.
    #[serde(default)]
    pub strike_type: Option<String>,
    /// Top-level updated cap strike. Present only on `metadata_updated` events, distinct from
    /// `additional_metadata.cap_strike` (emitted on market creation). Added 2026-06-18.
    #[serde(default)]
    pub cap_strike: Option<f64>,
    /// Top-level updated custom strike (custom/structured markets only). Present only on
...
pub struct WsEventLifecycle {
    ...
    /// Identifier for the exchange shard the event's markets live on. Marked required in the
    /// AsyncAPI spec (added 2026-07-30), but kept `Option` per this crate's convention of
    /// tolerating fields the exchange may omit in practice.
    #[serde(default)]
    pub exchange_index: Option<i64>,
    #[serde(default)]
    pub title: Option<String>,
