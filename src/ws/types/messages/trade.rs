...
pub struct WsTrade {
    ...
    /// 2026-08-13. Defaults to `false` so payloads predating the flag still
    /// parse, mirroring the REST `Trade.is_block_trade` field.
    #[serde(default)]
...
mod tests {
    ...
    #[test]
    fn ws_trade_is_block_trade_parses_and_defaults_false() {
        let with_flag = r#"{
            "trade_id":"t","market_ticker":"T","count_fp":"1",
            "yes_price_dollars":"0.10","no_price_dollars":"0.90",
            "is_block_trade":true,"ts":0,"ts_ms":0
        }"#;
        let trade: WsTrade = serde_json::from_str(with_flag).unwrap();
        ...
        let trade: WsTrade = serde_json::from_str(without_flag).unwrap();
        assert!(!trade.is_block_trade);
