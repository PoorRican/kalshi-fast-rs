...
mod tests {
    ...
    #[test]
    fn pyth_value_owned_and_ref_round_trip() {
        let json = r#"{
            "underlying_ticker": "BTCUSD",
            "value_usd": "68123.45670000",
            "source_ts_ms": 1753123456789,
            "received_at": 1753123456800
        }"#;

        let owned: WsPythValue = serde_json::from_str(json).unwrap();
        assert_eq!(owned.underlying_ticker, "BTCUSD");
        assert_eq!(owned.value_usd, "68123.45670000");
        assert_eq!(owned.source_ts_ms, 1753123456789);
        assert_eq!(owned.received_at, 1753123456800);
        ...
        assert_eq!(round_tripped.value_usd, "68123.45670000");
        assert_eq!(round_tripped.source_ts_ms, 1753123456789);
        assert_eq!(round_tripped.received_at, 1753123456800);
    }

    #[test]
    fn pyth_underlying_list_owned_and_ref_round_trip() {
        let json = r#"{
