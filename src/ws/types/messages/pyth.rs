...
/// Message payload for the `pyth_value` WebSocket channel: a deduplicated
/// real-time Pyth price update for a single underlying ticker. Added
/// 2026-07-23.
...
pub struct WsPythValue {
    ...
    /// When Kalshi received the upstream update (unix ms).
    pub received_at: i64,
}
...
/// underlying tickers recently observed on the stream.
#[derive(Debug, Clone, Deserialize)]
...
mod tests {
    ...
    #[test]
    fn ws_pyth_value_parses_owned_and_borrowed() {
        let json = r#"{
        ...
        assert_eq!(owned.received_at, 1710000000123);

        let borrowed: WsPythValueRef = serde_json::from_str(json).unwrap();
        let round_tripped = borrowed.into_owned();
        assert_eq!(round_tripped.underlying_ticker, owned.underlying_ticker);
        assert_eq!(round_tripped.value_usd, owned.value_usd);
    }

    #[test]
    fn ws_pyth_underlying_list_parses_owned_and_borrowed() {
        let json = r#"{"underlying_tickers": ["Metal.XAG/USD", "Metal.XAU/USD"]}"#;

        let owned: WsPythUnderlyingList = serde_json::from_str(json).unwrap();
        assert_eq!(
            owned.underlying_tickers,
            vec!["Metal.XAG/USD", "Metal.XAU/USD"]
        );

        let borrowed: WsPythUnderlyingListRef = serde_json::from_str(json).unwrap();
        assert_eq!(
            borrowed.into_owned().underlying_tickers,
            vec!["Metal.XAG/USD", "Metal.XAU/USD"]
        );
    }
}
