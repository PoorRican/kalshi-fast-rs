...
/// Message payload for the `pyth_value` WebSocket channel (type: "pyth_value").
///
/// Deduplicated Pyth price update for a single underlying ticker. Duplicate and
/// out-of-order source timestamps are dropped by the exchange independently per
/// underlying ticker.
...
///
/// Lists the underlying tickers observed on the Pyth stream in the last two
/// hours. Requesting it does not modify the subscription.
#[derive(Debug, Clone, Deserialize)]
