...
///
/// Requires an authenticated connection (the Pyth data feed is a paid
/// add-on; see the AsyncAPI `pyth_value` channel description).
...
/// Lists tickers observed on the stream within the last two hours; not a
/// complete catalog of every available feed.
#[derive(Debug, Clone, Deserialize)]
