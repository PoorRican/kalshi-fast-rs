...
mod tests {
    ...
    /// `subaccount` is present only when your side of the quote used a
    /// subaccount. Added to `quote_created` 2026-07-30 for parity with the
    /// existing `quote_accepted` / `quote_executed` field.
    #[test]
    fn quote_created_deserializes_subaccount() {
        let json = r#"{
            "quote_id": "q-1",
            "rfq_id": "r-1",
    ...
    #[test]
    fn quote_created_tolerates_missing_subaccount() {
        let json = r#"{
            "quote_id": "q-1",
            "rfq_id": "r-1",
            "quote_creator_id": "u-1",
