...
mod tests {
    ...
    fn subscription_tracker_apply_update_tracks_pyth_underlyings() {
        ...
        let params = WsSubscriptionParamsV2 {
            ...
            market_tickers: Some(vec!["UNTOUCHED".to_string()]),
        ...
        let add = WsUpdateSubscriptionParamsV2 {
            ...
            skip_ticker_ack: None,
            index_ids: None,
            underlying_tickers: Some(vec!["Metal.XAG/USD".to_string()]),
        };
        tracker.apply_update(&add);
        ...
        assert!(underlyings.contains(&"Metal.XAG/USD".to_string()));
        // Underlying actions must never touch tracked market targets.
        assert_eq!(
            updated.market_tickers,
            Some(vec!["UNTOUCHED".to_string()]),
            "underlying actions must not mutate market targets"
        );
        ...
        // `underlying_list` is a pure query: it must not mutate tracked state.
        let list = WsUpdateSubscriptionParamsV2 {
        ...
        let before = tracker.active.get(&9).cloned().unwrap();
        tracker.apply_update(&list);
        assert_eq!(tracker.active.get(&9), Some(&before));
    }
}
