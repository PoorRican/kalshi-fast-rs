...
impl SubscriptionTracker {
    ...
    pub(crate) fn apply_update(&mut self, update: &WsUpdateSubscriptionParamsV2) {
        ...
        if update.action.is_index_action() {
            ...
            // Pyth underlying actions only mutate the tracked underlying set
            // so that a reconnect resubscribes with the correct underlyings.
...
mod tests {
    ...
    fn subscription_tracker_apply_update_tracks_pyth_underlyings() {
        ...
        assert!(underlyings.contains(&"Metal.XPT/USD".to_string()));
