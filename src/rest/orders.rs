...
pub struct CancelOrderV2Params {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub subaccount: Option<u32>,
    /// Exchange shard index. If omitted, auto-routes when `market_ticker` is provided
    /// (default behavior since 2026-08-27); otherwise defaults to 0. Use `-1` to
...
/// Query params for Cancel All Orders (V2). Added 2026-08-27.
#[derive(Debug, Clone, Default, Serialize)]
pub struct CancelAllOrdersParams {
    /// Cancel only this subaccount's orders. Omit to match orders from any subaccount.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub subaccount: Option<u32>,
}
...
impl KalshiRestClient {
    ...
    ///
    /// Uses `BookSide` + single fixed-point `price`. This is the only order-creation
    /// endpoint; the legacy `/portfolio/orders` mutation surface was removed from the
    /// API 2026-06/2026-07.
    ///
    /// **Requires auth.**
    ...
    /// Cancel all resting event-market orders across every exchange shard. Added
    /// 2026-08-27. Newly placed orders may also be cancelled during the minute after
    /// the request.
    ///
    /// **Requires auth.**
