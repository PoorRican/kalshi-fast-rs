...
//! (including cancel-all), V2 batch operations, queue positions, order groups, and FCM
//! subtrader views.
//!
//! All endpoints require authentication. The legacy (non-V2) order-mutation endpoints
//! (`POST /portfolio/orders`, `DELETE /portfolio/orders/{id}`, amend, decrease, and the
//! batched create/cancel) have been removed from the API; use the `_v2` methods instead.
...
impl KalshiRestClient {
    ...
    /// Cancel all resting event-market orders for the authenticated member across every
    /// exchange shard. If `subaccount` is omitted, matching orders may come from any
