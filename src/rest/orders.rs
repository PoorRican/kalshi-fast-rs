...
//! order groups, and FCM subtrader views.
//!
//! Order mutation (create/amend/decrease/cancel, batch create/cancel) uses the
//! V2 event-order endpoints (`/portfolio/events/orders/*`) below. The legacy
//! `/portfolio/orders` mutation surface was removed upstream 2026 (Kalshi
//! announced deprecation 2026-06-18); this crate never re-adds it.
//!
...
pub struct Order {
    ...
    /// Exchange shard this order resides on. Added 2026-08-20.
    #[serde(default)]
    pub exchange_index: Option<u32>,
}
...
/// Query params for `PUT /portfolio/order_groups/{order_group_id}/limit`.
#[derive(Debug, Clone, Default, Serialize)]
...
pub struct GetFcmOrdersParams {
    /// Required unless `client_order_ids` is supplied. For an API key bound
    /// to a subtrader, defaults to the bound subtrader when omitted.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub subtrader_id: Option<String>,
    /// CSV of up to 100 client order IDs. Only orders created within the
    /// last 24 hours are searched. Required unless `subtrader_id` is
