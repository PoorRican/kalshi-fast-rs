//! Order endpoints: orders listing, single-order lookups, queue positions, order groups,
//! and FCM subtrader views for the legacy `/portfolio/orders*` surface (GET-only — the V1
//! create/amend/decrease/cancel/batch mutation endpoints were removed by Kalshi and have no
//! shim here), plus the V2 event-order mutation endpoints
//! (`/portfolio/events/orders/*`: create, cancel, amend, decrease, and batch create/cancel).
//!
...
pub struct GetOrdersParams {
    ...
    /// Filter results by exchange shard. Omit to return orders from all shards.
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// Added 2026-08-06 (order group limit updates support subaccounts).
#[derive(Debug, Clone, Default, Serialize)]
