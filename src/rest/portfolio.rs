...
pub struct GetBalanceParams {
    /// Scope the balance and portfolio value to one exchange index. Both
    /// values cover all exchange indexes when omitted. Added 2026-08-13.
...
/// A balance scoped to one exchange index.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
pub struct MarketPosition {
    ...
    /// Identifier for the exchange shard this market lives on. Added
    /// 2026-08-20; required by the spec, but kept `Option` for resilience.
    #[serde(default)]
    pub exchange_index: Option<i64>,
}
...
pub struct Settlement {
    ...
    /// Identifier for the exchange shard this market was settled on. Added
    /// 2026-08-20.
    #[serde(default)]
    pub exchange_index: Option<i64>,
}
...
pub struct GetPortfolioRestingOrderTotalValueResponse {
    ...
    /// Total resting order value broken down per exchange index, each
...
impl KalshiRestClient {
    ...
    ///
    /// Both `balance` and `portfolio_value` cover all exchange indexes
    /// unless `params.exchange_index` is set.
    ///
    /// **Requires auth.**
