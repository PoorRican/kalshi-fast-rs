...
//!
//! The lookup-history endpoint (`GET .../lookup`) was fully removed from the
//! Predictions REST API (2026-07-02); the ticker-pair lookup endpoint itself
//! (`PUT .../lookup`) still exists but is now deprecated upstream in favor of
//! RFQs.
...
impl KalshiRestClient {
    ...
    /// Look up an individual market in a multivariate event collection by
    /// selected legs.
    #[deprecated(
        since = "0.7.0",
        note = "predates RFQs; deprecated upstream, do not use for new integrations"
    )]
