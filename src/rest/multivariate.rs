...
//! This module exposes the collection read endpoints and the market-creation
//! endpoint.
//!
//! The `PUT /multivariate_event_collections/{collection_ticker}/lookup` endpoint
//! and its `GET` lookup-history counterpart were removed by the exchange on
//! 2026-08-06 (the `GET` variant had been fully deprecated on 2026-07-02) and no
//! longer exist in the OpenAPI spec. Use
//! [`KalshiRestClient::create_market_in_multivariate_event_collection`] to create
//! or resolve a combo market, or the communications (RFQ) APIs for quoting.
...
pub struct MultivariateEventCollection {
    ...
    /// Exchange shard the collection's series lives on. Added 2026-08-06;
    /// `None` for payloads predating the field.
    #[serde(default)]
...
impl KalshiRestClient {
    ...
    /// Create (or resolve) an individual market in a multivariate event collection.
    ///
    /// **Requires auth.**
    ///
    /// On an invalid leg combination the exchange returns a
    /// [`crate::ErrorResponse`] whose `code` is one of
    /// `conflicting_leg_outcomes`, `duplicated_legs`, or
    /// `invalid_market_combination`. Since 2026-07-30 `message` explains why the
    /// combination is invalid and `details` carries the offending market tickers
    /// as a comma-separated string when they are known.
