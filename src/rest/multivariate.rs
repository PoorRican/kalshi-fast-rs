...
//! This module exposes the CRUD endpoints and the ticker-pair selection type
//! used to create markets within a collection.
//!
//! The lookup endpoint (`GET`/`PUT .../{collection_ticker}/lookup`) and the
//! `multivariate`/`multivariate_lookup` WebSocket channel were removed from
//! the published OpenAPI/AsyncAPI specs (as of OpenAPI 3.28.0) and are no
//! longer modeled here. See `docs/spec-parity.md`.
...
pub struct MultivariateEventCollection {
    ...
    /// Exchange shard this collection's markets live on. Defaults to 0 if unspecified.
    #[serde(default)]
