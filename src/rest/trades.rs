...
pub struct GetHistoricalCutoffResponse {
    ...
    /// Cutoff based on settled-position archival time. Positions archived to
    /// the historical database before this timestamp must be accessed via
...
impl KalshiRestClient {
    ...
    /// are archived per whole event; use this endpoint for positions older
    /// than `market_positions_last_updated_ts` from `get_historical_cutoff`.
    /// Unsettled positions remain available via `get_positions`.
    ///
