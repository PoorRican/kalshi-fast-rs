...
/// GET /historical/positions query params. Positions are archived per whole
/// event; use this for settled positions older than the
/// `market_positions_last_updated_ts` cutoff on `GET /historical/cutoff`.
/// Added 2026-07-23.
#[derive(Debug, Clone, Default, Serialize)]
...
impl KalshiRestClient {
    ...
    /// List settled market/event positions archived to the historical
    /// database. Positions are archived per whole event; use this for
    /// positions older than `market_positions_last_updated_ts` on
    /// `GET /historical/cutoff`. Added 2026-07-23.
    ///
    /// **Requires auth.**
