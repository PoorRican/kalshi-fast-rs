...
pub struct GetHistoricalCutoffResponse {
    ...
    /// from the live data set before this timestamp are only available via
    /// [`KalshiRestClient::get_historical_positions`]. Added 2026-07-23.
    #[serde(default)]
...
impl KalshiRestClient {
    ...
    /// List settled market/event positions archived to the historical database.
    /// Positions are archived per whole event; use [`KalshiRestClient::get_historical_cutoff`]'s
    /// `market_positions_last_updated_ts` to determine the archival boundary. Requires auth.
