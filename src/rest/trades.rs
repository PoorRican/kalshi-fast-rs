...
pub struct GetHistoricalCutoffResponse {
    ...
    /// Cutoff based on position last-update time. Settled positions archived from
    /// the live data set before this timestamp are served through
    /// `GET /historical/positions`. Added 2026-07-23; absent from the upstream
    /// `required` list, so kept optional.
    #[serde(default)]
...
/// GET /historical/positions query params. Narrower than portfolio's
/// `GetPositionsParams` (this endpoint only supports `ticker`, `event_ticker`
/// (single), `limit`, and `cursor` — no `count_filter` or `subaccount`).
#[derive(Debug, Clone, Default, Serialize)]
...
impl KalshiRestClient {
    ...
    /// Get settled market/event positions archived to the historical database.
    /// Positions whose markets were archived before
    /// `market_positions_last_updated_ts` (see [`Self::get_historical_cutoff`])
    /// are available here. Unsettled positions are always available via
    /// [`Self::get_positions`]. Requires auth. Added 2026-07-23.
