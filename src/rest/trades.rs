...
impl KalshiRestClient {
    ...
    /// List settled positions archived to the historical database. Requires auth.
    ///
    /// Positions are archived per whole event: a settled event's positions move
    /// here together and are never split between this endpoint and
    /// `GET /portfolio/positions`. Use for positions older than
    /// `market_positions_last_updated_ts` from [`Self::get_historical_cutoff`].
    pub async fn get_historical_positions(
        &self,
        params: GetHistoricalPositionsParams,
    ) -> Result<crate::rest::portfolio::GetPositionsResponse, KalshiError> {
