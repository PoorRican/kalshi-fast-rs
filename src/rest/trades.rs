...
pub struct Trade {
    ...
    /// True if this trade was matched off-book as a block trade (e.g. via RFQ /
    /// negotiated block proposal). Added 2026-05-29. Kept `Option` for parse
    /// safety against historical trades that predate the field.
    #[serde(default)]
...
pub struct GetTradesParams {
    ...
    /// Filter by block trade status. Omit to return all trades, `true` for only
    /// block trades, `false` for only non-block trades. Added 2026-05-29.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub is_block_trade: Option<bool>,
}
