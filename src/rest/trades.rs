...
impl KalshiRestClient {
    ...
    /// `market_positions_last_updated_ts` (see [`get_historical_cutoff`]) are
    /// available here; a settled event's positions move here together and
    /// are never split between this endpoint and `GET /portfolio/positions`.
    ///
