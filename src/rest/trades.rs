...
/// Params for `GET /historical/positions`. Added 2026-07-23. Positions are
/// archived per whole event (a settled event's positions move to the
/// historical database together), so this endpoint has no `count_filter` or
/// `subaccount` params unlike `GET /portfolio/positions`.
#[derive(Debug, Clone, Default, Serialize)]
...
impl KalshiRestClient {
    ...
    /// older than the `market_positions_last_updated_ts` cutoff returned by
    /// [`Self::get_historical_cutoff`] live here rather than
    /// `GET /portfolio/positions`. Added 2026-07-23. Requires auth.
