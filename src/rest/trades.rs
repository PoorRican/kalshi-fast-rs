...
pub struct Trade {
    ...
    /// True if matched off-book as a block trade (e.g. via RFQ). Added 2026-06-01.
    /// OpenAPI marks this required; `#[serde(default)]` guards against pre-cutoff historical records.
    #[serde(default)]
...
pub struct GetTradesParams {
    ...
    /// Filter by block trade status. `None` = all trades; `Some(true)` = block only; `Some(false)` = non-block only.
    #[serde(skip_serializing_if = "Option::is_none")]
