...
pub struct GetHistoricalCutoffResponse {
    ...
    /// Cutoff based on settled-position archival time. Settled positions
    /// archived before this timestamp must be accessed via
    /// `GET /historical/positions`; unsettled positions are always
    /// available via `GET /portfolio/positions`. `Option` since older
    /// exchange snapshots may not yet include this field.
    #[serde(default)]
...
impl KalshiRestClient {
    ...
    /// List settled market positions that have been archived to the
    /// historical database (see `GET /historical/cutoff`'s
    /// `market_positions_last_updated_ts`). Unsettled positions are always
    /// available via [`Self::get_positions`].
    ///
    /// **Requires auth.**
