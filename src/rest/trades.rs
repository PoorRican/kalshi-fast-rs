...
pub struct GetHistoricalCutoffResponse {
    ...
    /// Cutoff based on market settlement time. Settled positions archived
    /// from the live data set before this timestamp must be accessed via
    /// `GET /historical/positions`. Unsettled positions are always available
    /// in `GET /portfolio/positions`. Added 2026-07-23.
    #[serde(default)]
...
impl KalshiRestClient {
    ...
    /// List settled market positions archived to the historical database.
    /// Positions are archived per whole event; unsettled positions remain
    /// available via [`get_positions`](Self::get_positions). Added 2026-07-23.
    ///
    /// **Requires auth.**
