...
pub struct GetExchangeStatusResponse {
    ...
    /// Per-index exchange status breakdown. Added 2026-07-02; absent when the
    /// per-index breakdown is unavailable. The top-level fields above reflect
    /// the default exchange index (0).
    #[serde(default)]
