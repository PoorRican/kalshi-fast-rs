...
pub struct GetExchangeStatusResponse {
    ...
    /// True if intra-exchange transfers are currently permitted. `Option`
    /// since older exchange snapshots (or a client talking to a slightly
    /// stale gateway during rollout) may not yet include the field.
    #[serde(default)]
    pub intra_exchange_transfers_active: Option<bool>,
    /// Per-exchange-index status breakdown. The top-level fields above
    /// reflect the default exchange index (0). Absent when the per-index
...
pub struct ExchangeIndexStatus {
    ...
    /// The live spec now marks this required, but it was added later
    /// (2026-08-13) than the rest of `ExchangeIndexStatus`, so it is kept
    /// `Option` in case older exchange snapshots omit it.
    #[serde(default)]
