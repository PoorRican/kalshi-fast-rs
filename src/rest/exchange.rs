...
/// Per-exchange-index breakdown of exchange status. Added with exchange
/// sharding (2026-08).
...
pub struct GetExchangeStatusResponse {
    ...
    /// True if intra-exchange transfers are currently permitted. Added with
    /// exchange sharding (2026-07).
    #[serde(default)]
    pub intra_exchange_transfers_active: Option<bool>,
    /// Per-exchange-index breakdown; the top-level fields above reflect the
    /// default exchange index (0). Absent when unavailable. Added with
    /// exchange sharding (2026-07).
    #[serde(default)]
