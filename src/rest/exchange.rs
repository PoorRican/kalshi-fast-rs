...
pub struct GetExchangeStatusResponse {
    ...
    /// True if intra-exchange transfers are currently permitted. Added 2026-06-26.
    #[serde(default)]
    pub intra_exchange_transfers_active: Option<bool>,
    #[serde(default)]
    pub exchange_estimated_resume_time: Option<String>,
    /// Per-index breakdown of exchange status. The top-level fields above reflect the
    /// default exchange index (0). Absent when the per-index breakdown is unavailable.
    /// Added 2026-06-26.
    #[serde(default)]
    pub exchange_index_statuses: Option<Vec<ExchangeIndexStatus>>,
}

/// Status of a single exchange index (shard). Currently only index `0` is in use.
#[derive(Debug, Clone, Deserialize, Serialize)]
