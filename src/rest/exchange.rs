...
pub struct GetExchangeStatusResponse {
    ...
    /// `#[serde(default)]` so responses from before the rollout still parse.
    #[serde(default)]
    pub intra_exchange_transfers_active: bool,
    /// Per-index breakdown of exchange status. The top-level fields above reflect
    /// the default exchange index (0). Absent when the per-index breakdown is
    /// unavailable. Added 2026-06-26.
    #[serde(default)]
...
///
/// See [`GetExchangeStatusResponse::exchange_index_statuses`].
#[derive(Debug, Clone, Deserialize, Serialize)]
