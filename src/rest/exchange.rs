...
//! trading is active (overall and per exchange-index shard), standard and
...
pub struct GetExchangeStatusResponse {
    ...
    /// True if intra-exchange transfers are currently permitted. Not present
    /// in the `required` list, so treated as optional for forward tolerance.
    #[serde(default)]
...
/// Status of a single exchange shard, as reported in
