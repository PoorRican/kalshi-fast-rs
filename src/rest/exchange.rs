...
//! trading is active (overall and per exchange shard), standard and
//! maintenance hours, and timestamps/fee changes useful for cache
//! invalidation. `GET /exchange/announcements` was removed from the API on
//! 2026-07-04.
...
/// Per-exchange-shard operational status, returned in `exchange_index_statuses`
/// on [`GetExchangeStatusResponse`]. Added 2026-07-02; `description` added 2026-08-07.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct ExchangeIndexStatus {
    pub exchange_index: u32,
    /// Human-readable description of this exchange shard. Added 2026-08-07.
...
pub struct GetExchangeStatusResponse {
    pub exchange_active: bool,
    pub trading_active: bool,
    /// Whether intra-exchange transfers are currently permitted on the default
    /// exchange index (0).
    #[serde(default)]
    pub intra_exchange_transfers_active: Option<bool>,
    #[serde(default)]
    pub exchange_estimated_resume_time: Option<String>,
    /// Per-shard status breakdown. Absent when unavailable. Added 2026-07-02.
    #[serde(default)]
