...
//! changes useful for cache invalidation. The `GET /exchange/announcements`
//! endpoint was removed by Kalshi 2026-07-04; see `docs/spec-parity.md`.
...
pub struct GetExchangeStatusResponse {
    ...
    /// True if intra-exchange transfers are currently permitted. Added 2026 alongside
    /// per-index exchange status.
    #[serde(default)]
    pub intra_exchange_transfers_active: Option<bool>,
    /// Status of each exchange index. The top-level fields above reflect the default
    /// exchange index (0). Absent when the per-index breakdown is unavailable. Added
    /// 2026-07-02.
    #[serde(default)]
