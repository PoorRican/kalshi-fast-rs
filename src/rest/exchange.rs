...
//!
//! `GET /exchange/announcements` was removed from the API on 2026-07-04; use
...
pub struct GetExchangeStatusResponse {
    ...
    /// Per-index breakdown; the top-level fields above reflect the default
    /// exchange index (0). Absent when the per-index breakdown is unavailable.
    /// Added 2026-07-02.
    #[serde(default)]
    pub exchange_index_statuses: Option<Vec<ExchangeIndexStatus>>,
}

/// Status of a single exchange shard. Added 2026-07-02.
#[derive(Debug, Clone, Deserialize, Serialize)]
