...
//! trading is active, standard and maintenance hours, and timestamps/fee changes
//! useful for cache invalidation. `GET /exchange/announcements` was removed from
//! the Predictions REST API 2026-07-04; use `GET /exchange/schedule` instead.
...
/// Per-exchange-index status entry on `GET /exchange/status`. Added 2026-07-02
/// (`exchange_index_statuses`); `description` added 2026-08-13.
...
pub struct GetExchangeStatusResponse {
    ...
    /// Per-index status breakdown; the top-level fields above reflect index 0. Added
    /// 2026-07-02. Absent when the per-index breakdown is unavailable.
