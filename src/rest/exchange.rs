...
//!
//! `GET /exchange/announcements` was removed from the API on 2026-07-04; the
//! corresponding client method and types were removed accordingly.
...
/// Per-exchange-index breakdown of operational status.
...
pub struct GetExchangeStatusResponse {
    ...
    /// True if intra-exchange transfers are currently permitted on the
    /// default exchange index (0). Added 2026-08-20.
    #[serde(default)]
    pub intra_exchange_transfers_active: Option<bool>,
    /// Per-exchange-index status breakdown. The top-level fields reflect the
