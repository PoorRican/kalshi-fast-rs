...
/// An exchange index's operational status. Present in
/// `GetExchangeStatusResponse::exchange_index_statuses` when the per-index
/// breakdown is available; the top-level `exchange_active` / `trading_active`
/// / `intra_exchange_transfers_active` fields on `GetExchangeStatusResponse`
/// reflect the default exchange index (0).
...
pub struct GetExchangeStatusResponse {
    ...
    /// Per-index status breakdown. Absent when unavailable. Added 2026-06.
    #[serde(default)]
