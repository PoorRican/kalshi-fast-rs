...
//! 2026-07-04; use `get_exchange_schedule` for exchange schedule information.
...
pub struct GetExchangeStatusResponse {
    ...
    /// Per-index status breakdown; the top-level fields reflect the default
    /// exchange index (0). Absent when the per-index breakdown is
