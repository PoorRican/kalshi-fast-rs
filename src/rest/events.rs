...
pub struct GetEventsParams {
    ...
    /// Event tickers, comma-separated (`EventTickersQuery`).
    #[serde(
...
pub struct EventData {
    ...
    #[serde(default)]
    pub category: Option<String>,
    /// Deprecated by Kalshi: no longer populated and always returns `false`.
    #[serde(default)]
    ...
    /// Official sources used to determine settlement for markets within this
    /// event. Required by the schema; defaults to empty when null/absent.
    #[serde(default, deserialize_with = "deserialize_null_as_empty_vec")]
    pub settlement_sources: Vec<SettlementSource>,
    /// Exchange shard this event lives on (2026-08 sharding rollout).
