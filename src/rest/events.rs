...
pub struct GetEventsParams {
    ...
    /// Comma-separated list of event tickers to filter by. Added to the
    /// OpenAPI spec alongside `min_updated_ts`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct EventData {
    ...
    /// event. Added to the top-level event payload 2026-06-18 (previously
    /// only available on `product_metadata`).
    #[serde(default, deserialize_with = "deserialize_null_as_empty_vec")]
