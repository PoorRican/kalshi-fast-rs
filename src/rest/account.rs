...
/// One volume-based API usage tier's earn/keep goals and the account's progress toward them.
/// See `GET /account/api_usage_level/volume_progress`. Added 2026-06-11.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
/// Trailing 30-day trading volume progress toward volume-based API usage tiers, for the
...
pub struct SubaccountBalance {
    pub subaccount_number: u32,
    /// Exchange index the balance is held on. Required upstream since 2026-07-02: a subaccount
    /// with funds on multiple indexes now appears as multiple entries rather than one combined
    /// row.
...
pub struct ApiKey {
    ...
    /// If set, the API key is restricted to this single sub-account (0-63) and may only read
...
pub struct CreateApiKeyRequest {
    ...
    pub scopes: Vec<String>,
    /// Restrict the API key to a single sub-account (0-63) that you own. A restricted key may
    /// only read and trade on that sub-account. Omit to leave the key unrestricted. Added
    /// 2026-07-02.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub subaccount: Option<u32>,
}
...
pub struct GenerateApiKeyRequest {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub scopes: Vec<String>,
    /// Restrict the API key to a single sub-account (0-63) that you own. Added 2026-07-02.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub subaccount: Option<u32>,
}
...
impl KalshiRestClient {
    ...
    /// Get the account's trailing 30-day trading volume progress toward volume-based API usage
    /// tiers (Expert, Premier, Paragon, Prime, Prestige) for the predictions lane. Added
    /// 2026-06-11.
    ///
    /// **Requires auth.**
    ...
    /// Grant a permanent Advanced API usage-level grant. Currently only the Predictions exchange
    /// instance is supported; requires at least 1 of the user's last 100 Predictions orders to
    /// have been created via API. Use [`Self::get_account_api_limits`] to inspect the resulting
    /// usage tier and grants. Added 2026-06-11.
    ///
    /// **Requires auth.**
