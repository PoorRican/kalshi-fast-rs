...
use crate::types::{
    ExchangeIndex, FixedPointDollars, deserialize_null_as_empty_vec, deserialize_string_or_number,
};
...
/// A volume-based API usage tier goal for the Predictions lane.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct AccountApiUsageLevelVolumeGoal {
    /// API usage level this goal is for (e.g. `"expert"`).
    pub level: String,
    /// Trailing-30d volume (fixed-point contracts) required to earn this level.
    pub earn_volume_goal_fp: String,
    /// Trailing-30d volume (fixed-point contracts) required to keep this level.
    pub keep_volume_goal_fp: String,
}

/// Trading-volume progress toward volume-based API usage tiers, computed as of
/// `computed_ts`.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct AccountApiUsageLevelVolumeProgress {
    /// Unix timestamp (seconds) this progress snapshot was computed.
    pub computed_ts: i64,
    /// Trailing 30-day volume (fixed-point contracts) ending at `computed_ts`.
...
pub struct SubaccountBalance {
    ...
    /// (per-index subaccount balances, exchange sharding rollout).
    #[serde(default)]
...
pub struct ApiKey {
    ...
    /// Sub-account (0-63) this key is restricted to; `None` when unrestricted.
    /// Added 2026-07-02.
    #[serde(default)]
...
pub struct GetApiKeysResponse {
    ...
    /// key requests expires; absent when the account has never attested.
    /// Added 2026-08-16.
    #[serde(default)]
...
pub struct CreateApiKeyRequest {
    ...
    pub scopes: Vec<String>,
    /// Restrict the key to a single sub-account (0-63) you own. Omit to leave
    /// the key unrestricted. Added 2026-07-02.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub subaccount: Option<u32>,
}
...
impl KalshiRestClient {
    ...
    /// (Predictions lane). Added 2026-06-11.
    ///
    /// **Requires auth.**
    ...
    /// Self-promote to the Advanced API usage tier. Requires at least one of the
