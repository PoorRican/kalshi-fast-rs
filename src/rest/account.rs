...
/// One volume-based API usage tier goal (Predictions/event_contract lane).
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct AccountApiUsageLevelVolumeGoal {
    /// API usage level for this volume goal (e.g. `"expert"`).
    pub level: String,
...
impl KalshiRestClient {
    ...
    /// Get the authenticated user's trailing 30d volume and volume-based API
    /// usage tier goals (Predictions/event_contract lane). Added 2026-06-11.
    ///
    /// **Requires auth.**
