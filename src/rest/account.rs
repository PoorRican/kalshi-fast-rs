...
pub struct AccountApiUsageLevelVolumeGoal {
    ...
    /// Trailing 30d volume required to keep this level.
    pub keep_volume_goal_fp: FixedPointCount,
}

/// Trailing 30d volume progress toward volume-based API usage tiers. Added 2026-06-11.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct AccountApiUsageLevelVolumeProgress {
    /// Unix timestamp (seconds) when this progress was computed.
    pub computed_ts: i64,
    /// Trailing 30-day trading volume, in fixed-point contract counts.
...
pub struct SubaccountBalance {
    ...
    pub updated_ts: i64,
    /// Exchange index this balance belongs to. A subaccount with funds on
...
pub struct CreateApiKeyRequest {
    ...
    pub scopes: Vec<String>,
    /// Restrict this key to a single sub-account (0-63) that you own. Omit to
...
impl KalshiRestClient {
    ...
    /// API usage tiers (Predictions/event_contract lane only).
    ///
    /// **Requires auth.**
