...
/// One Predictions volume-based API usage tier goal (earn/keep).
#[derive(Debug, Clone, Deserialize, Serialize)]
...
pub struct SubaccountBalance {
    ...
    /// with funds on multiple indexes now appears as multiple entries.
...
pub struct ApiKey {
    ...
    /// Sub-account (0-63) this key is restricted to; absent/`None` means the
...
impl KalshiRestClient {
    ...
    /// Get the authenticated user's trading volume progress toward
    ...
    /// Grant a permanent Advanced API usage-level grant, if the account
    /// qualifies (at least one of the user's last 100 Predictions orders was
    /// created via API).
    ///
    /// **Requires auth.**
