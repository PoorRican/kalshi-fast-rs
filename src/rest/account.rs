...
/// One volume-based API usage tier's goal, from
/// `GetAccountApiUsageLevelVolumeProgressResponse`. Added 2026-06-11.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
/// One computed snapshot of trailing 30-day volume progress toward
...
pub struct ApiKey {
    ...
    /// Subaccount this key is restricted to (absent when unrestricted).
    /// Added 2026-07-02.
    #[serde(default)]
    pub subaccount: Option<u32>,
    /// Unix timestamp (seconds) when this key's location attestation for API
    /// key requests expires. Once past, the key cannot trade Sports,
    /// Elections, and Entertainment markets. Absent if never attested.
    /// Added 2026-08-16.
    #[serde(default)]
