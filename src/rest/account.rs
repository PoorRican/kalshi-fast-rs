...
/// A single goal (earn/keep volume thresholds) for a volume-based API usage
/// level.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
/// tiers, computed as of `computed_ts`.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
pub struct CreateApiKeyRequest {
    ...
    pub scopes: Vec<String>,
    /// If set, restricts the key to a single sub-account (0-63) that you
    /// own. Omit to leave the key unrestricted. Added 2026-07-02.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub subaccount: Option<u32>,
}
...
impl KalshiRestClient {
    ...
    /// Advanced usage-level grant if at least one of the user's last 100
