...
pub struct AccountApiUsageLevelVolumeProgress {
    ...
    pub computed_ts: i64,
    /// Trailing 30-day volume ending at `computed_ts`, for the Predictions
    /// (event_contract) lane.
...
pub struct SubaccountBalance {
    pub subaccount_number: u32,
    /// Exchange index this balance is held on. Added 2026-07-02: a
    /// subaccount with funds on multiple indexes now appears as multiple
...
pub struct CreateApiKeyRequest {
    ...
    pub scopes: Vec<String>,
    /// Restrict the new key to a single sub-account (0-63). Omit to leave
...
impl KalshiRestClient {
    ...
    /// API usage tiers (Predictions/event_contract lane).
    ///
    /// **Requires auth.**
