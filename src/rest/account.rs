...
/// One volume-based API usage-tier goal. Added 2026-06-11.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
pub struct SubaccountBalance {
    pub subaccount_number: u32,
    /// Exchange shard the balance is held on. Now required by the spec
    /// (2026-07-02): a subaccount with funds on multiple exchange indexes
    /// returns one row per index. Kept `Option` to tolerate older payloads.
    #[serde(default)]
...
pub struct ApiKey {
    ...
    /// only read and trade on it. `None` means the key is unrestricted. Added 2026-07-02.
    #[serde(default)]
...
impl KalshiRestClient {
    ...
    /// earn/keep volume goals for each volume-based API usage tier.
    ///
    /// **Requires auth.**
    ...
    /// Self-promote to the Advanced API usage level. Grants a permanent
    /// Advanced API usage-level grant (currently Predictions only); requires
