...
pub struct AccountApiUsageLevelVolumeGoal {
    ...
    pub level: String,
    /// Trailing-30d volume required to newly earn this level.
...
pub struct SubaccountBalance {
    pub subaccount_number: u32,
    /// Exchange index the balance is held on. Added 2026-07: a subaccount
    /// now has one balance row per exchange index it holds funds on.
...
pub struct ApiKey {
    ...
    /// only read and trade on it. Absent/null means unrestricted. Added 2026-07.
    #[serde(default)]
...
pub struct CreateApiKeyRequest {
    ...
    pub scopes: Vec<String>,
    /// If set, restricts the key to this single sub-account (0-63) that you
    /// own. A restricted key may only read and trade on that sub-account.
    /// Added 2026-07.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub subaccount: Option<u32>,
}
...
impl KalshiRestClient {
    ...
    /// Advanced-level grant if at least one of the caller's last 100
    ...
    /// Get the caller's latest cron-computed trading volume progress toward
