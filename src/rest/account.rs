...
/// One goal on the path to a volume-based API usage tier.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct AccountApiUsageLevelVolumeGoal {
    /// API usage level this Predictions volume goal corresponds to (e.g. `"expert"`).
    pub level: String,
...
pub struct GetAccountApiUsageLevelVolumeProgressResponse {
    /// Volume progress for the predictions (`event_contract`) lane, across
    /// the volume-based public tiers (Expert, Premier, Paragon, Prime, Prestige).
    pub volume_progress: Vec<AccountApiUsageLevelVolumeProgress>,
}

/// Response for `POST /account/api_usage_level/upgrade`. Empty on success.
#[derive(Debug, Clone, Default, Deserialize, Serialize)]
pub struct UpgradeAccountApiUsageLevelResponse {}
...
pub struct SubaccountBalance {
    ...
    /// multiple indexes appears as multiple entries, one per index.
...
/// A transfer between exchange instances (e.g. Predictions <-> Margin).
///
/// `source`/`destination` are kept as `String` rather than an `ExchangeInstance`
/// enum (values: `event_contract`, `margined`) so the raw value round-trips
/// losslessly and tolerates future exchange-instance additions, matching
...
pub struct ApiKey {
    ...
    /// may only read and trade on it. Absent when the key is unrestricted.
    #[serde(default)]
...
impl KalshiRestClient {
    ...
    ///
    /// **Requires auth.**
    pub async fn upgrade_account_api_usage_level(
        &self,
    ) -> Result<UpgradeAccountApiUsageLevelResponse, KalshiError> {
    ...
    /// List intra-exchange account transfer history (e.g. Predictions <-> Margin).
    ...
    /// Fetch all pages of intra-exchange instance transfers using cursor pagination.
    ///
    /// **Requires auth.**
