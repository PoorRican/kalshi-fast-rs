...
/// A single volume-based API usage-level goal on
...
/// A single intra-exchange-instance account transfer.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct IntraExchangeInstanceTransfer {
    pub transfer_id: String,
    /// Source exchange instance (e.g. `"event_contract"`, `"margined"`). Kept
    /// as a raw string so future exchange-instance values round-trip
    /// losslessly, matching `ApiUsageLevelGrant.exchange_instance`.
    pub source: String,
    pub destination: String,
    pub source_exchange_shard: i64,
    pub destination_exchange_shard: i64,
    pub amount: FixedPointDollars,
    /// Transfer status (`"pending"` | `"complete"`). Kept as a raw string for
    /// the same reason as `source`/`destination`.
    pub status: String,
    pub created_ts: i64,
}
...
pub struct SubaccountBalance {
    ...
    /// with funds on multiple exchange indexes now appears as multiple
    /// entries rather than one combined row.
...
pub struct ApiKey {
    ...
    /// If set, the API key is restricted to this single subaccount (0-63) and
    /// may only read and trade on it. Absent/null means the key is
...
pub struct CreateApiKeyRequest {
    ...
    pub scopes: Vec<String>,
    /// Restrict the API key to a single subaccount (0-63) that you own.
...
impl KalshiRestClient {
    ...
    /// Get the account's trailing 30-day trading volume and the earn/keep
    /// volume goals for each volume-based API usage tier. Added 2026-06-11.
    ///
    /// **Requires auth.**
    ...
    /// Self-promote to the Advanced API usage tier. Grants a permanent
    /// Advanced API usage-level grant; requires at least 1 of the user's last
    /// 100 Predictions orders to have been created via API. Added 2026-06-11.
    ///
    /// **Requires auth.**
    ...
    /// Create a pager for iterating over intra-exchange account transfers
    /// page by page. Added 2026-08-13.
    ///
    ...
    /// Stream intra-exchange account transfers one by one. Added 2026-08-13.
    ///
    /// **Requires auth.**
