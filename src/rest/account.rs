...
/// Volume goal for a single API usage level in the volume-progress response.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
/// Trailing 30-day trading volume and per-tier goals, computed by a
/// scheduled job for the Predictions (event_contract) lane.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
pub struct SubaccountBalance {
    ...
    /// multiple indexes appears as multiple entries (2026-07-02).
    #[serde(default)]
...
pub struct ApiKey {
    ...
    /// If set, the API key is restricted to this single sub-account (0-63)
    /// and may only read and trade on it. Absent/null means the key is
    /// unrestricted (2026-07-02).
    #[serde(default)]
...
pub struct GetApiKeysResponse {
    ...
    /// lapsed. Absent when the account has never attested (2026-08-16).
    #[serde(default)]
...
pub struct CreateApiKeyRequest {
    ...
    /// leave the key unrestricted (2026-07-02).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub subaccount: Option<u32>,
}
...
impl KalshiRestClient {
    ...
    /// Get the authenticated user's trailing 30-day trading volume and
    /// earn/keep goals for volume-based API usage tiers (added 2026-06-11).
    ///
    /// **Requires auth.**
