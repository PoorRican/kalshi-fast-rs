...
pub struct GetQuotesParams {
    ...
    /// 2026-06-18 (replaces the removed `market_ticker`/`event_ticker` filters).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl KalshiRestClient {
    ...
    /// Get a quote by ID.
    ///
    /// Deprecated by Kalshi in favor of [`Self::get_rfq_quote`], which scopes
    /// the lookup to its RFQ. Still supported for now.
    ...
    ///
    /// Deprecated by Kalshi in favor of [`Self::delete_rfq_quote`], which
    /// scopes the deletion to its RFQ. Still supported for now.
    ...
    ///
    /// Deprecated by Kalshi in favor of [`Self::accept_rfq_quote`], which
    /// scopes the acceptance to its RFQ. Still supported for now.
    ...
    ///
    /// Deprecated by Kalshi in favor of [`Self::confirm_rfq_quote`], which
    /// scopes the confirmation to its RFQ. Still supported for now.
