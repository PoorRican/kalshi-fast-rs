...
pub struct Quote {
    ...
    /// True when the RFQ's target cost is principal-only and quote sizes were
    /// computed without reserving taker fees. Added 2026-09-10.
    #[serde(default)]
...
pub struct GetQuotesParams {
    ...
    /// Removed from the live `GET /communications/quotes` filter set
    /// 2026-06-20; the server now silently ignores it. Kept as a harmless
    /// no-op field for source compatibility. See `docs/spec-parity.md`.
    #[deprecated(note = "removed upstream 2026-06-20; the server ignores this filter")]
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct CreateRFQRequest {
    ...
    /// target cost / price), with taker fees charged on top. Defaults to
    /// `false` (target cost caps principal plus fees) when omitted. Added
    /// 2026-09-10.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub target_cost_excludes_fees: Option<bool>,
    pub rest_remainder: bool,
...
impl KalshiRestClient {
    ...
    /// Get a quote by ID.
    #[deprecated(note = "use get_rfq_quote (GET /communications/rfqs/{rfq_id}/quotes/{quote_id})")]
    ...
    #[deprecated(
        note = "use delete_rfq_quote (DELETE /communications/rfqs/{rfq_id}/quotes/{quote_id})"
    )]
    ...
    #[deprecated(
        note = "use accept_rfq_quote (PUT /communications/rfqs/{rfq_id}/quotes/{quote_id}/accept)"
    )]
    ...
    #[deprecated(
        note = "use confirm_rfq_quote (PUT /communications/rfqs/{rfq_id}/quotes/{quote_id}/confirm)"
    )]
    ...
    /// Get a quote scoped to its RFQ. Added 2026-07-09; preferred over
    /// `get_quote`, which the live API now deprecates in favor of this path.
    ...
    /// Delete a quote scoped to its RFQ (it can no longer be accepted).
    /// Added 2026-06-25; preferred over `delete_quote`.
    ...
    /// Accept a quote scoped to its RFQ. The quoter must then confirm.
    /// Added 2026-06-25; preferred over `accept_quote`.
    ...
    /// Confirm a quote scoped to its RFQ, starting the order-execution timer.
    /// Rate limits are more favorable than `confirm_quote`. Added 2026-06-25;
    /// preferred over `confirm_quote`.
