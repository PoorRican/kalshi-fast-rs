...
impl KalshiRestClient {
    ...
    #[deprecated(
        note = "use get_rfq_quote instead; the exchange returns 404 for this path once the quote-ID-only lookup is fully removed"
    )]
    ...
    /// [`Self::get_quote`]; the quote must belong to `rfq_id` or the exchange
    /// returns 404.
