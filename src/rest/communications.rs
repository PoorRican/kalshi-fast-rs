...
pub struct GetQuotesParams {
    ...
    /// Filter quotes with `updated_ts` >= this Unix timestamp (seconds).
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Filter to quotes from a specific user (pass `"self"` for authenticated user).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl KalshiRestClient {
    ...
    ///
    /// Prefer this over `delete_quote` when you have the RFQ ID available,
    /// as it provides unambiguous routing.
