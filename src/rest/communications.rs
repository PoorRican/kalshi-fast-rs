...
pub struct GetRFQsParams {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub status: Option<String>,
    /// Deprecated: use `user_filter: Some("self")` for the authenticated user instead.
    #[deprecated(note = "use `user_filter` instead")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub creator_user_id: Option<String>,
    /// Pass `"self"` to filter to RFQs created by the authenticated user.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub user_filter: Option<String>,
}
...
pub struct CreateRFQRequest {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub subaccount: Option<u32>,
    /// When `true`, size derived quotes as principal only (contracts = target
    /// cost / price), charging the taker fee on top of the target cost instead
    /// of folding fees into the target cost. Requires `target_cost_dollars` (or
    /// `target_cost_centi_cents`). Added 2026-09-10.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl KalshiRestClient {
    ...
    /// Deprecated 2026-06-25: use [`Self::get_rfq_quote`] instead. RFQ quotes
    /// are no longer guaranteed queryable without an `rfq_id` scope.
    ...
    /// belong to `rfq_id`. Added 2026-06-25 (replaces [`Self::get_quote`]).
    ...
    /// Delete a quote scoped to its RFQ. Added 2026-06-25 (replaces [`Self::delete_quote`]).
    ...
    /// Accept a quote scoped to its RFQ. Added 2026-06-25 (replaces [`Self::accept_quote`]).
    ...
    /// Confirm a quote scoped to its RFQ. Added 2026-06-25 (replaces [`Self::confirm_quote`]).
