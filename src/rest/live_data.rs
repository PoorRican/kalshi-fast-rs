...
/// Live data keyed by event ticker (e.g. crypto price charts, commodity timeseries, weather
/// observations), as returned by `GET /live_data/events/{event_ticker}`. Added 2026-07-30.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct EventLiveData {
    ...
    #[serde(default)]
    pub details: Map<String, Value>,
    /// Present for crypto live data. True when the event has matured and the payload is a
...
pub struct GetEventLiveDataParams {
    /// Optional chart range hint (e.g. `15min`, `1h`, `1d`). When the underlying live data type
    /// supports it, restricts the returned timeseries to the requested window.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl KalshiRestClient {
    ...
    /// Get live data for an event by its event ticker (crypto price charts, commodity price
    /// timeseries, weather observations, etc). The `live_data_type` field names the schema of
    /// `details`.
