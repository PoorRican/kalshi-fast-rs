...
/// weather observations, etc. `live_data_type` names the schema of `details`.
/// Added 2026-07-30.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct EventLiveData {
    ...
    #[serde(default)]
    pub details: Map<String, Value>,
    /// True when the event has matured and the payload is a frozen
    /// historical snapshot (crypto live data only).
    #[serde(default)]
...
/// A single per-station audit reading on a [`WeatherIndexPoint`], present
/// only with `detailed=true`.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
pub struct WeatherIndexPoint {
    ...
    /// Present only on points produced by the historical backfill that seeds
    /// a city's series before it went live. `synoptic_latency` means the
    /// receipt-deadline test used the Synoptic ingest latency in place of
...
/// A published weather-index configuration record.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
impl KalshiRestClient {
    ...
    /// configuration plus every weekly calibration). Added 2026-08-31.
