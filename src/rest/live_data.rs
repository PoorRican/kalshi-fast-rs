...
pub struct GetEventLiveDataParams {
    /// Chart range hint (e.g. `15min`, `1h`, `1d`), when the live data type supports it.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub range: Option<String>,
}

/// Per-station audit reading on a [`WeatherIndexPoint`] (only with `detailed=true`).
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct WeatherIndexStationReading {
    pub station_id: String,
    /// Disposition: `ok`, `missing`, `late`, a QC rejection (`range`,
    /// `rate_spatial`, `extreme`), or `pending` (raw reading on an
    /// `incomplete` minute).
    pub code: String,
    #[serde(default)]
...
pub struct WeatherIndexPoint {
    ...
    pub status: String,
    /// Number of accepted member stations. Absent on `incomplete` points.
    #[serde(default)]
    pub contributors: Option<i64>,
    /// Present only on points from the historical backfill seeding a newly
    /// listed city's series. `synoptic_latency` means the receipt deadline
    /// was judged against `observation_time + Synoptic ingest latency`
    /// instead of Kalshi's local receipt clock; such points are not
    /// settlement-eligible. Absent on canonical points.
    #[serde(default)]
...
pub struct WeatherIndexCalibration {
    ...
    /// Absent on records not derived from a calibration window (the launch config).
    #[serde(default)]
...
pub struct GetWeatherIndexCalibrationsResponse {
    ...
    /// Always `celsius`.
    pub units: String,
    /// Ascending by effective time; append-only and never trimmed.
