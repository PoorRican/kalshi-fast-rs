...
pub struct GetEventLiveDataParams {
    /// Chart range hint (e.g. `"15min"`, `"1h"`, `"1d"`), when the underlying
    /// live data type supports it.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct EventLiveData {
    /// Names the schema of `details` (e.g. crypto price charts, commodity price
...
pub struct WeatherIndexStationReading {
    ...
    /// `"rate_spatial"`, `"extreme"`), or `"pending"` (incomplete minute).
    pub code: String,
    #[serde(default)]
...
/// One minute-resolution point on the Kalshi Weather Index.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct WeatherIndexPoint {
    ...
    /// points, which have no canonical value yet.
    #[serde(default)]
    pub v: Option<f64>,
