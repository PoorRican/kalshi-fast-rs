...
/// Response for `GET /live_data/events/{event_ticker}`. Serves event-keyed
/// live data such as crypto price charts, commodity price timeseries, and
/// weather observations. The `type` field on `live_data` names the schema of
/// `details`, which is kept generic to decouple from that schema.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
// ---------------------------------------------------------------------------
// Kalshi Weather Index (added 2026-08-20 / 2026-08-31)
// ---------------------------------------------------------------------------
...
pub struct WeatherIndexPoint {
    ...
    /// Per-station audit readings (only with `detailed=true`), kept generic
    /// since this diagnostic payload is not central to trading use cases.
    #[serde(default)]
    pub stations: Option<Vec<Value>>,
}

/// GET /live_data/weather/{city}/calibrations response. Added 2026-08-31.
#[derive(Debug, Clone, Deserialize, Serialize)]
...
/// One weather-index configuration record. `stations` is kept generic
/// (`Vec<Value>`) since the per-station weight/offset breakdown is
/// diagnostic detail, not central to trading use cases.
#[derive(Debug, Clone, Deserialize, Serialize)]
